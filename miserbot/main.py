...
cur.execute(“SELECT COALESCE(SUM(plays),0) FROM songs”)
total_plays = cur.fetchone()[0]
cur.execute(“SELECT COUNT(*) FROM fan_locations”)
mapped = cur.fetchone()[0]
cur.execute(“SELECT COUNT(*) FROM auctions WHERE status=‘active’”)
auctions = cur.fetchone()[0]
if uid == OWNER_ID:
cur.execute(“SELECT COALESCE(SUM(points),0) FROM users”)
total_pts = cur.fetchone()[0]
cur.execute(“SELECT COUNT(*) FROM users WHERE is_supporter=TRUE”)
supporters = cur.fetchone()[0]
cur.execute(“SELECT COUNT(*) FROM stripe_sessions WHERE status=‘completed’”)
stripe_done = cur.fetchone()[0]
cur.execute(“SELECT COUNT(*) FROM artist_submissions WHERE status=‘pending’”)
...
cur = conn.cursor()
try:
cur.execute(“INSERT INTO voice_wall (telegram_id, username, file_id) VALUES (%s,%s,%s) RETURNING id”,
...
f”Reward: +{POINTS[‘submit_track’]} MiserCoins\n\nType /cancel to go back.”)
...
if "#vault" in caption:
    ...
    try:
        cur.execute(
            "INSERT INTO vault_songs (title, file_id, required_points) VALUES (%s,%s,%s) RETURNING id",
            (title, file_id, req_pts))
    ...
    if tag in caption:
        ...
        try:
            if dest == "songs":
                ...
                if cur.fetchone():
                    await update.message.reply_text(f"Song '{title}' already exists.")
                    return
...
if dest == “songs”:
cur.execute(
“INSERT INTO songs (title, file_id) VALUES (%s,%s) ON CONFLICT (title) DO NOTHING RETURNING id”,
(title, file_id))
else:
cur.execute(f”INSERT INTO {dest} (title, file_id) VALUES (%s,%s) RETURNING id”, (title, file_id))
row = cur.fetchone()
...
cur.execute(
“SELECT id, title, description, current_bid, top_username, ends_at FROM auctions WHERE status=‘active’ AND ends_at>NOW() ORDER BY ends_at”)
...
f”BAZRAGOD drops auctions for exclusive songs, NFTs, feature slots, and video calls.\n\nStay tuned.”)
return
...
try:
cur.execute(
“SELECT a.title, ab.amount, a.current_bid, a.top_bidder, a.ends_at FROM auction_bids ab JOIN auctions a ON ab.auction_id=a.id WHERE ab.telegram_id=%s ORDER BY ab.bid_at DESC LIMIT 10”,
(uid,))
bids = cur.fetchall()
finally:
release_db(conn)
...
try:
cur.execute(
“SELECT id, title, current_bid, starting_bid, ends_at FROM auctions WHERE id=%s AND status=‘active’”,
(auction_id,))
...
return True
cur.execute(
“SELECT id, current_bid, top_bidder FROM auctions WHERE id=%s AND status=‘active’ AND ends_at>NOW()”,
(session[“auction_id”],))
auction = cur.fetchone()
if not auction:
...
old_top = auction[2]
cur.execute(
“UPDATE auctions SET current_bid=%s, top_bidder=%s, top_username=%s WHERE id=%s”,
(bid_amount, uid, name, session[“auction_id”]))
cur.execute(
“INSERT INTO auction_bids (auction_id, telegram_id, username, amount) VALUES (%s,%s,%s,%s)”,
...
try:
cur.execute(
“INSERT INTO downloads (telegram_id, song_id, purchased) VALUES (%s,%s,TRUE) ON CONFLICT DO NOTHING”,
(fan_id, song_id))
...
args = context.args
if len(args) < 4:
await update.message.reply_text(“Usage: /create_auction <title> <desc> <bid> <hours>”)
return
...
try:
cur.execute(
“INSERT INTO auctions (title, description, starting_bid, current_bid, ends_at) VALUES (%s,%s,%s,%s,%s) RETURNING id”,
...
args = context.args
if len(args) < 4:
await update.message.reply_text(“Usage: /add_event <title> <desc> <YYYY-MM-DD> <location>”)
return
title = args[0].replace(”*”, “ “)
description = args[1].replace(”*”, “ “)
try:
...
ticket_url = args[4] if len(args) > 4 else None
conn = get_db()
cur = conn.cursor()
try:
cur.execute(
“INSERT INTO events (title, description, event_date, location, ticket_url) VALUES (%s,%s,%s,%s,%s) RETURNING id”,
(title, description, event_date, location, ticket_url))
eid = cur.fetchone()[0]
conn.commit()
finally:
release_db(conn)
...
if panel == "music":
    ...
    try:
        cur.execute(
            "SELECT artist_name, song_title FROM artist_submissions WHERE status='approved' ORDER BY RANDOM() LIMIT 5")
    ...
    await query.message.reply_text(
        ...
        f"Miserbot is the sovereign digital music nation of BAZRAGOD.\n\n"
        f"No labels. No middlemen. Direct connection between artist and fans.\n\n"
        f"Platform: Telegram Super App\nRadio: 24/7 BazraGod Radio\nNation: Parish 14\nBrain: MAXIMUS AI\n\n"
...
if context.user_data.get("shop_dedication"):
    ...
    if deduct_points(uid, cost, "shop_dedication"):
        ...
        try:
            cur.execute(
                "INSERT INTO dedications (telegram_id, username, message) VALUES (%s,%s,%s) RETURNING id",
                (uid, name, text))
...
try:
cur.execute(
“SELECT id, title, top_bidder, top_username, current_bid FROM auctions WHERE status=‘active’ AND ends_at<NOW()”)

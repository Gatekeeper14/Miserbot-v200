...
RADIO_PROMOS = [
    ...
    "Invite your friends. Parish 14 grows stronger.",
...
MISSIONS = [
    ...
    "Check your fan profile and share it",
    "Visit the music store and explore",
]
...
Platform lives entirely inside Telegram.
Catalog: 15 tracks. Nation: Parish 14.

When reading astrocartography: speak like a calm documentary narrator.
Be clear, powerful, and inspiring — never fearful.

Keep responses concise for Telegram — max 3 paragraphs.
End every response with a power statement."""

ASTRO_SYSTEM_PROMPT = """You are MAXIMUS, astrocartography reader for BAZRAGOD.

When given birth details and current location, generate a powerful
astrocartography reading. Speak like a calm documentary narrator.

Structure your reading:
1. Current location energy (1 paragraph)
2. Best locations for creativity and music (1 paragraph)
3. Best locations for business and wealth (1 paragraph)
4. Power advice for right now (1 sentence)

Style: confident, clear, inspiring. No fear language.
End with a sovereign power statement."""
...
def award_points(telegram_id: int, action: str, username: str = None) -> int:
    ...
    try:
        ...
        cur.execute(
            "SELECT points FROM fans WHERE telegram_id = %s", (telegram_id,)
        )
        row = cur.fetchone()
        if row:
            cur.execute(
...
def register_fan(telegram_id: int, username: str, referrer_id: int = None) -> bool:
    ...
    try:
        cur.execute(
            "SELECT telegram_id FROM fans WHERE telegram_id = %s", (telegram_id,)
        )
        if cur.fetchone():
            return False
        cur.execute("""
...
async def intro_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT points, tier FROM fans WHERE telegram_id = %s", (uid,)
        )
        row = cur.fetchone()
    finally:
        release_db(conn)
...
async def play_song(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT title, file_id FROM songs WHERE id = %s", (song_id,)
        )
        song = cur.fetchone()
    finally:
        release_db(conn)
    ...
    if song:
        ...
        await maximus_speak(
            context, uid,
            f"Now playing {song[0]}. BAZRAGOD. Parish 14 Nation."
        )
...
async def play_beat_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT title, file_id FROM beats WHERE id = %s", (beat_id,)
        )
        beat = cur.fetchone()
    finally:
        release_db(conn)
...
async def play_drop_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT title, file_id FROM drops WHERE id = %s", (drop_id,)
        )
        drop = cur.fetchone()
    finally:
        release_db(conn)
...
async def radio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if slot == "promo":
        ...
        await maximus_speak(context, uid, f"BazraGod Radio — {now}. {promo}")
        await update.message.reply_text(
    ...
    if slot == "drop":
        ...
        if item:
            await maximus_speak(
                context, uid,
                f"BazraGod Radio — {now}. Special drop incoming. {item[0]}. Parish 14."
            )
    ...
    if slot == "beat":
        ...
        if item:
            await maximus_speak(
                context, uid,
                f"BazraGod Radio. {now}. This beat is called {item[0]}. Feel it."
            )
            await update.message.reply_audio(
                item[1],
                caption=f"📻 BazraGod Radio — {now}\n\n🥁 BEAT: {item[0]}\n\n+{pts} pts",
    ...
    dj_lines = [
        ...
        f"Straight from the sovereign. {now} on BazraGod Radio. {song[0]}. Feel it.",
        f"Independent music for independent minds. {song[0]} by BAZRAGOD. Right now.",
    ]
...
async def booking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"📅 BOOK BAZRAGOD\n\n"
        f"For shows, features, events, and collaborations.\n\n"
        f"Contact:\n"
        f"{BOOKING_EMAIL}\n\n"
        f"Include:\n"
        f"• Event type\n"
...
async def fan_radar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute("""
            ...
            FROM fan_locations
            GROUP BY country ORDER BY fans DESC LIMIT 10
        """)
        rows = cur.fetchall()
    ...
    if not rows:
        await update.message.reply_text(
            "📡 FAN RADAR\n\n"
            "No fan locations mapped yet.\n\n"
            "Share your location to put your city on the map! 🌍"
        )
        return

    text = (
        f"📡 PARISH 14 FAN RADAR\n\n"
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute("""
            ...
            FROM fans ORDER BY points DESC LIMIT 10
        """)
        rows = cur.fetchall()
    finally:
        release_db(conn)
...
async def my_points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT points, invites, tier FROM fans WHERE telegram_id = %s", (uid,)
        )
        row = cur.fetchone()
        cur.execute(
...
async def my_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            ...
            "FROM fans WHERE telegram_id = %s", (uid,)
        )
        row = cur.fetchone()
        cur.execute(
            "SELECT COUNT(*) FROM fans WHERE points > "
...
async def daily_mission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT completed FROM missions "
            "WHERE telegram_id = %s AND mission_date = %s",
            (uid, today),
        )
        row = cur.fetchone()
        if not row:
            cur.execute(
                "INSERT INTO missions (telegram_id, mission_date) "
...
async def refer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT invites, tier FROM fans WHERE telegram_id = %s", (uid,)
        )
        row = cur.fetchone()
    finally:
        release_db(conn)
...
async def astro_reading(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not openai_client:
        await update.message.reply_text(
            "🪐 ASTRO READING\n\nMAXIMUS AI offline. OPENAI_API_KEY required."
        )
        return
    ...
    await update.message.reply_text(
        ...
        "I will map your planetary energy.\n\n"
        "Step 1 of 3\n\n"
...
async def astro_input_handler(
    ...
    if step == "birth_time":
        ...
        await update.message.reply_text(
            "✅ Birth time saved.\n\n"
            "Step 3 of 3\n\n"
            "Enter your birth city and current city:\n\n"
    ...
    if step == "location":
        ...
        await update.message.reply_text(
            "🪐 MAXIMUS is reading your stars...\n\n"
            "Stand by. 👑"
        )

        try:
            parts        = text.split(",")
            birth_city   = parts[0].strip() if len(parts) > 0 else "Unknown"
            ...
            try:
                cur.execute("""
                    ...
                    (telegram_id, birth_date, birth_time, birth_city, current_city)
                    VALUES (%s, %s, %s, %s, %s)
                    ON CONFLICT (telegram_id) DO UPDATE
            ...
            await update.message.reply_text(
                ...
                f"{reading}\n\n"
                f"{'═' * 20}\n"
                f"+{pts} pts — MAXIMUS sees your path. 👑"
            )
            await maximus_speak(context, uid, reading[:500])

        except Exception as e:
            await update.message.reply_text(
                f"🪐 Reading error: {str(e)}\n\nTry again later."
            )
        finally:
...
async def ai_assistant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not openai_client:
        await update.message.reply_text(
            "🤖 MAXIMUS is offline. OPENAI_API_KEY not set."
        )
        return
...
async def radar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute("""
            SELECT COALESCE(country, 'Unknown'), COUNT(*) as fans
            FROM fan_locations
            GROUP BY country ORDER BY fans DESC LIMIT 15
        """)
        rows = cur.fetchall()
        cur.execute("SELECT COUNT(*) FROM fan_locations")
        total = cur.fetchone()[0]
...
async def _broadcast_to_all(
    context, text: str, speak: bool = False
) -> int:
    conn = get_db()
    cur  = conn.cursor()
    try:
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    astro_sessions.pop(update.effective_user.id, None)
...
async def _delete_from(update, context, table):
    ...
    try:
        cur.execute(
            f"DELETE FROM {table} WHERE id = %s RETURNING title", (row_id,)
        )
        row = cur.fetchone()
        conn.commit()
    finally:
        release_db(conn)
    if row:
        await update.message.reply_text(f"🗑 Deleted: {row[0]}")
    else:
...
async def router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Astro session intercept — highest priority
    if uid in astro_sessions:
        handled = await astro_input_handler(update, context)
        if handled:
            return

    # AI session intercept
...
if __name__ == "__main__":
    ...
    init_db()
    print("╔══════════════════════════════════════╗")
    print("║   I.A.A.I.M.O — MISERBOT v6000      ║")

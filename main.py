...
async def play_song(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if song:
        ...
        await maximus_speak(context, uid, f"Now playing {song[0]}. BAZRAGOD.")
...
async def radio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if slot == "drop":
        ...
        if item:
            await maximus_speak(
                context, uid,
                f"BazraGod Radio — {now}. Special drop. {item[0]}. Parish 14."
            )
    ...
    if slot == "beat":
        ...
        if item:
            await maximus_speak(
                context, uid,
                f"BazraGod Radio. {now}. This beat is {item[0]}. Feel it."
            )
            await update.message.reply_audio(
                item[1],
                caption=f"📻 BazraGod Radio — {now}\n\n🥁 BEAT: {item[0]}\n\n+{pts} pts",

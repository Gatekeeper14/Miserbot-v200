"""
╔══════════════════════════════════════════════════════════════╗
║         I.A.A.I.M.O — MASTER SYSTEM v14.000                 ║
║  Independent Artists Artificial Intelligence Music Ops       ║
║  Bot:     Miserbot       Nation:  Parish 14                  ║
║  Owner:   BAZRAGOD                                           ║
║                                                              ║
║  v14 GOD MODE UPGRADES:                                      ║
║  🎛️ Professional Radio Rotation Engine                       ║
║     Song → Drop → Beat → Song → Drop → Beat                  ║
║     Ad Break every 4 songs → Announcement after ad           ║
║  🥁 5 Beats auto-seeded on first boot                        ║
║  🎤 9 Drops auto-seeded on first boot                        ║
║  📢 1 Announcement auto-seeded on first boot                 ║
║  🎵 Wine Gal added to song catalog                           ║
║  📻 Channel Radio Loop uses full professional rotation        ║
║  🎙️ In-chat Radio uses full professional rotation            ║
║  🔊 New System Intro audio                                   ║
╚══════════════════════════════════════════════════════════════╝
"""
...
# Updated system intro — new audio
...
# Radio timing
RADIO_SONG_DELAY        = 200   # seconds between songs in channel loop
RADIO_BEAT_DELAY        = 120   # seconds after a beat
RADIO_DROP_DELAY        = 35    # seconds after a drop
RADIO_AD_DELAY          = 25    # seconds after an ad message
RADIO_ANNOUNCE_DELAY    = 30    # seconds after an announcement
...
# 1 announcement seeded on first boot
SEED_ANNOUNCEMENTS = [
...
def init_db():
    ...
    try:
        ...
        conn.commit()
        print("I.A.A.I.M.O DATABASE READY — v14.000")
...
# ╔══════════════════════════════════════════════════════════════╗
# ║     🎛️ PROFESSIONAL RADIO ROTATION ENGINE v14               ║
# ║                                                              ║
# ║  Pattern per cycle:                                          ║
# ║  Song → Drop → Song → Drop → Beat → Song → Drop →           ║
# ║  Song → AD_BREAK → Announcement → repeat                    ║
# ║                                                              ║
# ║  This is how real radio stations are programmed.             ║
# ╚══════════════════════════════════════════════════════════════╝

def build_professional_playlist() -> list:
    """
    Builds a full professional radio rotation from the database.

    Rotation pattern (per song cycle):
      Song 1  → Drop
      Song 2  → Drop → Beat
      Song 3  → Drop
      Song 4  → Drop → Beat → AD_BREAK → Announcement (if available)

    This creates a natural radio feel:
    music never stops, drops keep energy up,
    beats give texture, ads/announcements are spaced cleanly.
    """
    conn = get_db()
    cur  = conn.cursor()
    try:
...
# ╔══════════════════════════════════════════════════════════════╗
# ║       📻 24/7 CHANNEL RADIO LOOP — PROFESSIONAL ROTATION     ║
# ╚══════════════════════════════════════════════════════════════╝
...
async def channel_radio_loop():
    """
    Continuously broadcasts to RADIO_CHANNEL_ID using
    the full professional rotation: songs, drops, beats,
    ad breaks, and announcements.
    """
...
def register_fan(telegram_id: int, username: str, referrer_id: int = None) -> bool:
    ...
    try:
        ...
        if cur.fetchone():
            return False
        cur.execute("INSERT INTO fans (telegram_id, username, referrer_id) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING", (telegram_id, username, referrer_id))
        if referrer_id:
...
async def _play_next_for_user(uid, name, pts, listeners, msg, context):
    """Dispatcher: routes each playlist item type to its handler."""
    item = get_next_playlist_item(uid)
    dj   = get_current_dj()
    now  = datetime.now().strftime("%I:%M %p")

    if item["type"] == "empty":
        await msg.reply_text("📻 Radio loading... Upload songs to the catalog first.", reply_markup=main_menu)
        return
    ...
    if item["type"] == "beat" and item["file_id"]:
        await dj_speak(context, uid, f"BazraGod Radio. {now}. Feel this. {item['title']}. Parish 14.")
        await msg.reply_audio(
            item["file_id"],
            caption=(
                f"🥁 BAZRAGOD BEAT — {item['title']}\n"
...
async def mood_radio_handler(uid: int, text: str, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if song:
        ...
        await update.message.reply_audio(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❤️ Like", callback_data=f"like:{sid}"), InlineKeyboardButton("💰 Donate", callback_data=f"donate:{sid}")]]),
        )
        await maybe_prophecy(uid, name, context)
    return True
...
async def handle_audio_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not audio:
        return
    title   = audio.title or audio.file_name or (update.message.caption or "").strip() or "Untitled Track"
    ...
    await update.message.reply_text(f"🎚️ CLASSIFY UPLOAD\n\nTitle: {title}\n\nWhat type is this audio?", reply_markup=keyboard)
...
async def merch_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(f"👕 PARISH 14 ORDER\n\nItem:  {item_name}\nPrice: ${price}\n\nAfter payment send admin:\n• Your size\n• Shipping address\n• Payment proof\n\nParish 14. 👑", reply_markup=keyboard)
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    # Build current rotation preview
    playlist = build_professional_playlist()
    ...
    channel_status = f"📻 Channel Radio: {'ACTIVE ✅' if radio_loop_running else 'STANDBY'}"
    channel_id_str = RADIO_CHANNEL_ID or 'NOT SET'

    await update.message.reply_text(
        f"👑 I.A.A.I.M.O ADMIN PANEL v14.000\n"
        f"══════════════════════════════\n\n"
        f"{channel_status}\n"
        f"Channel ID: {channel_id_str}\n\n"
...
def health():
    ...
    return (
        f"I.A.A.I.M.O ONLINE — PARISH 14 NATION v14.000 | "
...
if __name__ == "__main__":
    ...
    # Print startup rotation preview
    playlist = build_professional_playlist()
    print("╔══════════════════════════════════════════════╗")
    print("║   I.A.A.I.M.O — MISERBOT v14.000            ║")
    print("║   Owner:      BAZRAGOD                       ║")
    print("║   Nation:     Parish 14                      ║")
    print(f"║   Songs:      {len(SEED_SONGS)} tracks                       ║")
    print(f"║   Beats:      {len(SEED_BEATS)} beats                         ║")
    print(f"║   Drops:      {len(SEED_DROPS)} drops                         ║")
    print(f"║   Playlist:   {len(playlist)} items total               ║")
    print(f"║   Channel:    {RADIO_CHANNEL_ID or 'NOT SET':<30}║")
    print("║   Languages:  6 ACTIVE                       ║")
    print("║   Rotation:   PROFESSIONAL ENGINE ACTIVE     ║")
    print("║   Status:     ONLINE                         ║")
    print("╚══════════════════════════════════════════════╝")

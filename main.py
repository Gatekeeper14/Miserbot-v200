...
async def radio(update,context):
    ...
    if song:
        ...
        dj_lines=[
            f"You are listening to BazraGod Radio. Next up {song[0]}",
            f"This is I.A.A.I.M.O Radio. The time is {now}. Playing {song[0]}",
            f"No label. No middleman. BazraGod Radio presents {song[0]}"
        ]

        dj=random.choice(dj_lines)

        await maximus_speak(context,update.effective_user.id,dj)

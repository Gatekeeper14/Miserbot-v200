...
async def channel_radio_loop(app):
    ...
    while True:
        try:
            ...
            if item["type"] == "ad":
                await app.bot.send_message(chat_id=channel_id, text=f"BazraGod Radio {now}\n\n{random.choice(AD_MESSAGES)}\n\nt.me/{BOT_USERNAME}")
                await asyncio.sleep(RADIO_AD_DELAY)
            elif item["type"] in ("announcement", "drop", "beat") and item["file_id"]:
                await app.bot.send_audio(chat_id=channel_id, audio=item["file_id"], caption=f"{item['title']}\nBazraGod Radio {now}\n\nt.me/{BOT_USERNAME}")
                ...
                await app.bot.send_audio(chat_id=channel_id, audio=item["file_id"],
...
async def bg_tasks(application):
    while True:
        try:
            conn = get_db(); cur = conn.cursor()
            try:
...
@flask_app.route("/")
def health():
    return jsonify({"status": "ONLINE", "platform": "I.A.A.I.M.O", "version": "18.000"})
...
def api_vault(uid):
    try:
        ...
        for vid, title, req_pts in items:
            vault.append({
                "id": vid, "title": title, "required_points": req_pts,
                "unlocked": vid in unlocked,
                "can_unlock": fan_pts >= req_pts
            })
...
def stripe_webhook():
    ...
    if event["type"] == "checkout.session.completed":
        asyncio.run_coroutine_threadsafe(handle_stripe_payment(event["data"]["object"], app.bot), loop)
    return "ok"

def run_flask():
    flask_app.run(host="0.0.0.0", port=int(os.environ.get("API_PORT", 8081)), use_reloader=False)

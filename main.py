import os, re, time, random, asyncio, threading, json, struct
...
async def generate_song_metadata(title):
    if not openai_client: return {"genre": "Dancehall", "mood": "Energetic", "description": f"{title} by BAZRAGOD"}
    try:
        response = openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "system", "content": "You are a music metadata generator. Respond only with JSON."},
                {"role": "user", "content": f"Generate metadata for a song called '{title}' by BAZRAGOD. Return JSON with keys: genre, mood, description, year. Keep description under 100 chars."}],
            max_tokens=150)
        text = response.choices[0].message.content.strip()
        text = text.replace("```json", "").replace("```", "").strip()
        return json.loads(text)
    except Exception:
        return {"genre": "Dancehall", "mood": "Energetic", "description": f"{title} by BAZRAGOD", "year": "2025"}

async def generate_album_art(title, song_id):
    if not openai_client: return None
    try:
        prompt = f"Album cover art for a song called '{title}' by BAZRAGOD. Black and gold aesthetic. Jamaican music culture. Sovereign. Premium. Abstract. No text."
        ...
        print(f"Image gen error: {e}"); return None

def embed_metadata_into_audio(audio_data, title, metadata, artwork_data=None, fan_passport=None):
    if not MUTAGEN_OK: return audio_data
    try:
        ...
        tags.add(TPE1(encoding=3, text="BAZRAGOD"))
        tags.add(TALB(encoding=3, text="I.A.A.I.M.O — Parish 14 Nation"))
        tags.add(TCON(encoding=3, text=metadata.get("genre", "Dancehall")))
        desc = metadata.get("description", "")
        if fan_passport:
            desc += f" | Parish 14 Collector: {fan_passport}"
        tags.add(COMM(encoding=3, lang="eng", desc="", text=desc))
        if artwork_data:
            tags.add(APIC(encoding=3, mime="image/jpeg", type=3, desc="Cover", data=artwork_data))
        out = BytesIO()
        tags.save(out)
        out.seek(0)
        return out.read()
    except Exception as e:
        print(f"Metadata embed error: {e}"); return audio_data
        ...
        if product_type == "single_song":
            ...
            if song_id:
                ...
                try:
                    ...
                    cur.execute("SELECT title, file_id, artwork_data FROM songs WHERE id=%s", (song_id,))
                    song = cur.fetchone()
                    cur.execute("SELECT passport_number FROM users WHERE telegram_id=%s", (uid,))
                    fan = cur.fetchone()
                    conn.commit()
                finally:
                    release_db(conn)
                if song:
                    title, file_id, artwork_data = song
                    passport = fan[0] if fan else None
                    metadata = await generate_song_metadata(title)
                    try:
                        audio_file = await bot.get_file(file_id)
                        audio_buf = BytesIO()
                        await audio_file.download_to_memory(audio_buf)
                        audio_data = audio_buf.getvalue()
                        enhanced = embed_metadata_into_audio(audio_data, title, metadata, artwork_data, passport)
                        enhanced_buf = BytesIO(enhanced)
                        enhanced_buf.name = f"{title}.mp3"
                        await bot.send_audio(uid, enhanced_buf,
                            caption=f"DOWNLOAD DELIVERED\n\n{title}\nBAZRAGOD\n\nParish 14 Collector Edition\nPassport: {passport or 'P14'}\n\nYours to keep. Forever.")
                        if artwork_data:
                            await bot.send_photo(uid, BytesIO(artwork_data),
                                caption=f"NFT COLLECTIBLE ART\n\n{title}\nBAZRAGOD x Parish 14\n\nThis artwork is yours.")
                    except Exception:
                        await bot.send_audio(uid, file_id, caption=f"DOWNLOAD DELIVERED\n\n{title}\nBAZRAGOD\n\nYours to keep. Parish 14 Nation.")
...
async def channel_radio_loop(application):
    ...
    while True:
        try:
            ...
            if item["type"] == "ad":
                await application.bot.send_message(chat_id=channel_id, text=f"BazraGod Radio {now}\n\n{random.choice(AD_MESSAGES)}\n\nt.me/{BOT_USERNAME}")
                await asyncio.sleep(RADIO_AD_DELAY)
            elif item["type"] in ("announcement", "drop", "beat") and item["file_id"]:
                await application.bot.send_audio(chat_id=channel_id, audio=item["file_id"], caption=f"{item['title']}\nBazraGod Radio {now}\n\nt.me/{BOT_USERNAME}")
                ...
                await application.bot.send_audio(chat_id=channel_id, audio=item["file_id"],
...
async def play_song_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("SELECT title, file_id, plays, likes, artwork_data FROM songs WHERE id=%s", (song_id,))
        song = cur.fetchone()
        if song: cur.execute("UPDATE songs SET plays=plays+1 WHERE id=%s", (song_id,)); conn.commit()
    finally:
        release_db(conn)
    if not song: return
    title, file_id, plays, likes, artwork = song; plays += 1
    ...
    if artwork:
        try:
            await q.message.reply_photo(BytesIO(artwork), caption=f"Collector Art — {title}")
        except Exception: pass
...
async def buy_song_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(f"BUY SONG\n\n{song[0]}\nBAZRAGOD\n\nPrice: ${SONG_PRICE}\n\nIncludes: Collector Edition with AI artwork embedded.\n\nBefore you pay - support the mission with $1?", reply_markup=InlineKeyboardMarkup(kb))
...
async def cart_checkout_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(f"CHECKOUT\n\n{count} song(s)\nTotal: ${price}\n\nEach song includes Collector Edition with AI artwork.\n\nBefore you pay - support the mission with $1?", reply_markup=InlineKeyboardMarkup(kb))
...
async def cmd_store(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("BAZRAGOD NETWORK\n\nStore and Services.",
        reply_markup=InlineKeyboardMarkup([
...
async def store_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "music": await cmd_music(update, context)
    elif action == "beats": await cmd_beats(update, context)
    elif action == "donate1":
...
async def service_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(f"Pay ${price:,} via Stripe", url=url)]] if url else [[InlineKeyboardButton("CashApp", url=CASHAPP)], [InlineKeyboardButton("PayPal", url=PAYPAL)]]
    await q.message.reply_text(f"SERVICE BOOKING\n\n{service_name}\nPrice: ${price:,}\n\nBAZRAGOD team contacts you within 24 hours.", reply_markup=InlineKeyboardMarkup(kb))
...
async def merch_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(f"Pay ${price} via Stripe", url=url)]] if url else [[InlineKeyboardButton("CashApp", url=CASHAPP)], [InlineKeyboardButton("PayPal", url=PAYPAL)]]
    await q.message.reply_text(f"PARISH 14 ORDER\n\nItem: {item_name}\nPrice: ${price}\nSizes: M / L / XL\n\nAfter payment send size and shipping address", reply_markup=InlineKeyboardMarkup(kb))
...
async def cmd_skills(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("SKILLS HARVEST\n\nWhat is your skill?\n\nExamples: Design, Video, Photography, Promotion, Web Dev\n\nType your skill name now.")

async def cmd_volunteer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "VOLUNTEER MISSIONS\n\nEarn MiserCoins by contributing to the nation.\n\nShare posts, create fan videos, translate content, promote BazraGod Radio.\n\nEach mission earns +100 MiserCoins.",
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    title = audio.title or audio.file_name or caption.replace("#song","").replace("#beat","").replace("#drop","").replace("#announce","").strip() or "Untitled"
    ...
    if "#vault" in caption:
        ...
        if check_duplicate(file_id, title, "vault_songs"):
            await update.message.reply_text(f"DUPLICATE — {title!r} already in Vault."); return
        conn = get_db(); cur = conn.cursor()
        try:
    ...
    for tag, (dest, label) in tag_map.items():
        if tag in caption:
            if check_duplicate(file_id, title, dest):
                await update.message.reply_text(f"DUPLICATE — {title!r} already exists."); return
            conn = get_db(); cur = conn.cursor()
            try:
                ...
                release_db(conn)
            await update.message.reply_text(f"{label.upper()} ADDED\n\nID: {new_id}\nTitle: {title}\n\nGenerating AI artwork and metadata...")
            invalidate_cache(); pl = build_playlist(); save_queue(pl)
            if dest == "songs":
                upload_sessions[uid] = new_id
                asyncio.ensure_future(generate_and_embed_artwork(new_id, title, update))
            return
    await update.message.reply_text("Add caption tag:\n#song #beat #drop #announce #vault\n\nOr send #picture to assign art to last song.")

async def generate_and_embed_artwork(song_id, title, update):
    try:
        await update.message.reply_text(f"Generating AI artwork for {title}...")
        image_data = await generate_album_art(title, song_id)
        if image_data:
            metadata = await generate_song_metadata(title)
            conn = get_db(); cur = conn.cursor()
            try:
                cur.execute("UPDATE songs SET genre=%s, description=%s WHERE id=%s",
                    (metadata.get("genre","Dancehall"), metadata.get("description",""), song_id))
                conn.commit()
            except Exception: pass
            finally:
                release_db(conn)
            await update.message.reply_photo(BytesIO(image_data),
                caption=f"AI ARTWORK GENERATED\n\n{title}\nBAZRAGOD\n\nEmbedded in song metadata.\nFans receive this as NFT collectible on purchase.")
        else:
            await update.message.reply_text(f"Artwork generation failed for {title}. Song uploaded without artwork.")
    except Exception as e:
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not song_id:
        ...
        try:
            ...
            row = cur.fetchone()
            if row: song_id = row[0]; song_title = row[1]
        finally:
            release_db(conn)
    if not song_id:
        await update.message.reply_text("No recent song found. Upload a song first with #song."); return
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("SELECT title FROM songs WHERE id=%s", (song_id,))
        row = cur.fetchone(); song_title = row[0] if row else "Unknown"
        cur.execute("UPDATE songs SET artwork_data=%s WHERE id=%s", (image_data, song_id))
        conn.commit()
    finally:
        release_db(conn)
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"ADMIN PANEL v18.000\n\nRadio: {'ACTIVE' if radio_loop_running else 'STANDBY'}\n\n"
        f"UPLOAD TAGS\n#song — auto generates AI artwork + metadata\n#beat #drop #announce #vault\n#picture — assign art to last song\n\n"
        f"COMMANDS\n/start_radio\n/premiere id\n/list_songs\n/delete_song id\n/list_vault\n/delete_vault id\n"
        f"/vault_unlock uid single|bundle\n/unlock_download uid song_id\n/activate_supporter uid\n"
...
async def cmd_start_radio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    asyncio.ensure_future(channel_radio_loop(app))
    await update.message.reply_text(f"Radio STARTED. Broadcasting to {RADIO_CHANNEL_ID}.")
...
async def cmd_shoutout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sent = await _do_broadcast(f"SHOUTOUT FROM BAZRAGOD\n\nBig up {args[0]} real Parish 14 energy.\n\nBAZRAGOD Network.")
...
async def _do_broadcast(text):
    ...
    sent = 0
    for (fid,) in fans:
        try: await app.bot.send_message(fid, text); sent += 1
        except Exception: pass
    return sent
...
async def cmd_list_songs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("SELECT id, title, plays, likes FROM songs ORDER BY id"); rows = cur.fetchall()
    finally:
        release_db(conn)
...
async def cmd_vault_unlock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: await context.bot.send_message(fan_id, "VAULT ACCESS GRANTED\n\nGo to Secret Vault.\n\nParish 14.")
    except Exception: pass
...
async def cmd_unlock_download(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        release_db(conn)
    if song:
        await update.message.reply_text(f"Unlocked for {fan_id}: {song[0]}")
...
async def cmd_add_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _do_broadcast(f"NEW EVENT\n\n{title}\n{description}\nDate: {event_date.strftime('%d %B %Y')}\nLocation: {location}")
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(uid):
        await update.message.reply_text(
            f"BAZRAGOD NETWORK STATS v18.000\n\n"
            f"Total Fans:   {total_fans:,}\nSupporters:   {supporters}\nMiserCoins:   {total_pts:,}\n"
            f"Songs:        {total_songs}\nBeats:        {total_beats}\nTotal Plays:  {total_plays:,}\n"
            f"Downloads:    {total_sales}\nStripe Sales: {stripe_sales}\nFans Mapped:  {mapped}")
    else:
        await update.message.reply_text(
...
async def cmd_weekly(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Total Fans:    {total_fans:,}\nNew This Week: {new_fans}\n"
        f"Total Coins:   {total_pts:,}\nStripe Sales:  {stripe_done}\n\n"
        f"TOP FANS\n{fans_text}\n\nTOP SONGS\n{songs_text}")
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"BAZRAGOD NETWORK HELP\n\n/start /music /beats /radio /cart /vault /store\n"
        f"/passport /coins /leaderboard /missions /invite\n"
        f"/radar /skills /volunteer /events /social /donate\n"
        f"/maximus /stats /help\n\nBooking: {BOOKING_EMAIL}\nBot: @{BOT_USERNAME}\n\nParish 14 Nation. BAZRAGOD.")
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in skill_sessions:
        ...
        if step == "skill_name":
            ...
            await update.message.reply_text("Great! Now describe your skill:"); return
            ...
            try: await app.bot.send_message(OWNER_ID, f"NEW SKILL #{sid}\n\nFan: @{name} ({uid})\nSkill: {skill_name}\n{text}")
            except Exception: pass
            return
    ...
    routes = {
        "MUSIC": cmd_music, "STORE": cmd_store, "SOCIAL": cmd_social,
        "MAXIMUS AI": cmd_maximus, "BazraGod Radio": cmd_radio,
        "My Passport": cmd_passport, "Secret Vault": cmd_vault, "Help": cmd_help,
...
@flask_app.route("/")
def health():
    return jsonify({"status": "ONLINE", "platform": "BAZRAGOD Network", "nation": "Parish 14", "version": "18.000"})
...
def api_catalog():
    try:
        conn = get_db(); cur = conn.cursor()
        try:
            cur.execute("SELECT id, title, plays, likes FROM songs ORDER BY plays DESC")
            songs = [{"id": r[0], "title": r[1], "plays": r[2], "likes": r[3], "price": SONG_PRICE} for r in cur.fetchall()]
            cur.execute("SELECT id, title, plays FROM beats ORDER BY id")
            beats = [{"id": r[0], "title": r[1], "plays": r[2], "price": 50} for r in cur.fetchall()]
        finally:
            release_db(conn)
        return jsonify({"songs": songs, "beats": beats})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@flask_app.route("/api/passport/<int:uid>")
def api_passport(uid):
    try:
        conn = get_db(); cur = conn.cursor()
        try:
            cur.execute("SELECT username, points, invites, tier, city, country, joined_at, is_supporter, passport_number FROM users WHERE telegram_id=%s", (uid,))
            ...
            release_db(conn)
        return jsonify({
            "username": username, "points": points, "invites": invites,
            "tier": tier, "city": city, "country": country,
            "is_supporter": is_sup, "passport_number": pnum or f"P14-{uid % 100000:05d}",
            "global_rank": rank, "downloads": downloads,
...
def api_leaderboard():
    try:
        conn = get_db(); cur = conn.cursor()
        try:
            cur.execute("SELECT username, points, tier FROM users ORDER BY points DESC LIMIT 20")
            rows = [{"username": r[0] or "Anonymous", "points": r[1], "tier": r[2]} for r in cur.fetchall()]
        finally:
            release_db(conn)
...
def api_stats():
    try:
        ...
        try:
            ...
            release_db(conn)
        return jsonify({"fans": fans, "songs": songs, "total_plays": plays, "fans_mapped": mapped, "sales": sales})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@flask_app.route("/api/vault/<int:uid>")
def api_vault(uid):
    try:
        conn = get_db(); cur = conn.cursor()
        try:
            ...
            release_db(conn)
        vault = [{"id": vid, "title": title, "required_points": req_pts,
                  "unlocked": vid in unlocked, "can_unlock": fan_pts >= req_pts}
                 for vid, title, req_pts in items]
        return jsonify({"vault": vault, "fan_coins": fan_pts})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
async def post_init(application):
    ...
    if RADIO_CHANNEL_ID:
        asyncio.create_task(channel_radio_loop(application))
...
def main():
    ...
    init_db()
    print("=" * 50)
    print("BAZRAGOD NETWORK v18.000")
    print("SOVEREIGN ARTIST PLATFORM")
    print("Bot: @BazragodMiserbot_bot")
    print("Nation: Parish 14")
    print("NFT Artwork: ENABLED")
    print("Status: ONLINE")
    print("=" * 50)

    flask_thread = threading.Thread(target=run_flask, daemon=True)
    flask_thread.start()
    print(f"API running on port {os.environ.get('PORT', 8080)}")

    application = Application.builder().token(BOT_TOKEN).post_init(post_init).build()

    application.add_handler(CommandHandler("start", cmd_start))
    application.add_handler(CommandHandler("menu", lambda u, c: u.message.reply_text("Main Menu", reply_markup=main_menu)))
    ...
    application.add_handler(CommandHandler("beats", cmd_beats))
    ...
    application.add_handler(CommandHandler("profile", cmd_passport))
    ...
    application.add_handler(CommandHandler("refer", cmd_invite))
    application.add_handler(CommandHandler("radar", cmd_radar))
    application.add_handler(CommandHandler("skills", cmd_skills))
    application.add_handler(CommandHandler("volunteer", cmd_volunteer))
    ...
    application.add_handler(CommandHandler("maximus", cmd_maximus))
    application.add_handler(CommandHandler("stats", cmd_stats))
    application.add_handler(CommandHandler("weekly", cmd_weekly))
    application.add_handler(CommandHandler("admin", cmd_admin))
    application.add_handler(CommandHandler("start_radio", cmd_start_radio))
    application.add_handler(CommandHandler("broadcast", cmd_broadcast))
    application.add_handler(CommandHandler("shoutout", cmd_shoutout))
    application.add_handler(CommandHandler("announce", cmd_announce))
    application.add_handler(CommandHandler("premiere", cmd_premiere))
    ...
    application.add_handler(CommandHandler("list_vault", cmd_list_vault))
    application.add_handler(CommandHandler("delete_vault", cmd_delete_vault))
    application.add_handler(CommandHandler("vault_unlock", cmd_vault_unlock))
    application.add_handler(CommandHandler("unlock_download", cmd_unlock_download))
    application.add_handler(CommandHandler("activate_supporter", cmd_activate_supporter))
    application.add_handler(CommandHandler("add_event", cmd_add_event))

    application.add_handler(CallbackQueryHandler(lang_cb, pattern="^lang:"))
    application.add_handler(CallbackQueryHandler(entry_step2, pattern="^entry:step2"))
    application.add_handler(CallbackQueryHandler(entry_step3, pattern="^entry:step3"))
    application.add_handler(CallbackQueryHandler(entry_agreed, pattern="^entry:agreed"))
    application.add_handler(CallbackQueryHandler(entry_gate_done, pattern="^entry:gate_done"))
    ...
    application.add_handler(CallbackQueryHandler(beat_cb, pattern="^beat:"))
    application.add_handler(CallbackQueryHandler(buy_beat_cb, pattern="^buy_beat:"))
    ...
    application.add_handler(CallbackQueryHandler(radio_next_cb, pattern="^radio:next"))
    application.add_handler(CallbackQueryHandler(vault_item_cb, pattern="^vault:"))
    application.add_handler(CallbackQueryHandler(vault_pay_cb, pattern="^vault_pay:"))
    application.add_handler(CallbackQueryHandler(store_cb, pattern="^store:"))
    application.add_handler(CallbackQueryHandler(service_cb, pattern="^service:"))
    application.add_handler(CallbackQueryHandler(merch_cb, pattern="^merch:"))
    application.add_handler(CallbackQueryHandler(leaderboard_cb, pattern="^lb:"))
    application.add_handler(CallbackQueryHandler(mission_complete_cb, pattern="^mission:"))
    application.add_handler(CallbackQueryHandler(volunteer_cb, pattern="^volunteer:"))
    application.add_handler(CallbackQueryHandler(action_cb, pattern="^action:"))

...
from config import BOT_TOKEN, RADIO_CHANNEL_ID, OWNER_ID, STRIPE_WEBHOOK_SECRET, OWNER_ID as OID
...
from services.economy import (
    ...
    run_daily_drip, process_stake_maturity
)
from services.radio import start_channel_radio
...
async def wisdom_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    import random
    from config import QUOTES, POINTS
...
async def fitness_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    from config import FITNESS_MSG, POINTS
    from services.economy import award_points
    uid = update.effective_user.id
    name = update.effective_user.username or str(uid)
...
async def social_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    from config import SOFT_GATE, POINTS
    from services.economy import award_points
    uid = update.effective_user.id
    name = update.effective_user.username or str(uid)
...
async def community_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    from config import BOT_USERNAME, PARISH_LOUNGE, RADIO_CHANNEL_LINK
    from telegram import InlineKeyboardMarkup, InlineKeyboardButton
    uid = update.effective_user.id
...
async def tour_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    from database import get_db, release_db
    from telegram import InlineKeyboardMarkup, InlineKeyboardButton
    from config import BOOKING_EMAIL
    conn = get_db(); cur = conn.cursor()
    try:
...
async def booking_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    from config import BOOKING_TERMS
    await update.message.reply_text(BOOKING_TERMS)

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    from config import BOOKING_EMAIL, BOT_USERNAME
    await update.message.reply_text(
        f"PARISH 14 HELP\n\n"
        f"PANELS\n"
...
async def voice_wall_submit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id
    name = update.effective_user.username or str(uid)
    if not context.user_data.get("voice_wall_active"): return
...
async def handle_audio_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not uid == OWNER_ID:
        if submission_sessions.get(uid):
            ...
            conn = get_db(); cur = conn.cursor()
            try:
                cur.execute("INSERT INTO artist_submissions (telegram_id, username, artist_name, song_title, file_id) VALUES (%s,%s,%s,%s,%s) RETURNING id", (uid, name, artist_name, song_title, file_id))
    ...
    import re
    tag_map = {
        "#song": ("songs", "Song"), "#beat": ("beats", "Beat"), "#drop": ("drops", "Drop"),
...
async def upload_classify_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query.from_user.id == OWNER_ID: return
    parts = query.data.split(":")
    dest = parts[1]; file_id = parts[2]; title = ":".join(parts[3:])
    if dest == "vault":
...
async def start_radio_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not RADIO_CHANNEL_ID:
        await update.message.reply_text("RADIO_CHANNEL_ID not set in Railway."); return
    if radio_loop_running:
        await update.message.reply_text("Radio loop already running."); return
...
async def _broadcast(context, text):
    ...
    for (fid,) in fans:
        try:
            await context.bot.send_message(fid, text); sent += 1
        except: pass
    return sent
...
async def premiere_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sent = await _broadcast(context, msg)
    await update.message.reply_text(f"Premiere sent to {sent} fans. Song set to Hot rotation.")

async def vault_unlock_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID: return
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("Usage: /vault_unlock <uid> <single|bundle>"); return
...
async def unlock_download_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("Usage: /unlock_download <uid> <song_id>"); return
...
async def activate_supporter_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    fan_id = int(args[0])
    from services.supporters import activate_supporter
    from services.economy import award_points
...
async def list_songs_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        text += f"[{r[0]}] {rb.get(r[5],'')} {r[1]}\n{heat_score(r[3],r[4],r[2])} {r[2]:,} plays  {r[3]} likes\n"
...
async def delete_song_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from database import get_db, release_db
    from services.radio import invalidate_cache, build_playlist, save_queue
    conn = get_db(); cur = conn.cursor()
    try:
...
async def approve_submission_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if row:
            ...
            conn.commit(); invalidate_cache(); pl = build_playlist(); save_queue(pl)
        else:
            conn.commit()
    finally:
        release_db(conn)
    if row:
        await update.message.reply_text(f"Submission #{sub_id} approved. {row[1]} {row[2]} added to radio.")
        try:
...
async def approve_voice_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("UPDATE voice_wall SET status='approved' WHERE id=%s RETURNING telegram_id", (int(args[0]),))
        row = cur.fetchone(); conn.commit()
    finally:
        release_db(conn)
    if row:
        await update.message.reply_text(f"Voice #{args[0]} approved.")
        try:
...
async def create_auction_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 4:
        await update.message.reply_text("Usage: /create_auction <title> <description> <starting_bid> <hours>"); return
    title = args[0].replace("_"," "); description = args[1].replace("_"," ")
    starting_bid = int(args[2]); hours = int(args[3])
    from datetime import datetime, timedelta
    from database import get_db, release_db
    ends_at = datetime.now() + timedelta(hours=hours)
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("INSERT INTO auctions (title, description, starting_bid, current_bid, ends_at) VALUES (%s,%s,%s,%s,%s) RETURNING id", (title, description, starting_bid, starting_bid, ends_at))
        aid = cur.fetchone()[0]; conn.commit()
    finally:
        release_db(conn)
    await update.message.reply_text(f"AUCTION CREATED\n\nID: {aid}\nTitle: {title}\nStarting bid: {starting_bid:,} coins\nEnds: {ends_at.strftime('%d/%m %H:%M')}")
    await _broadcast(context, f"NEW AUCTION LIVE\n\n{title}\n{description}\nStarting bid: {starting_bid:,} MiserCoins\nEnds: {ends_at.strftime('%d/%m/%Y %H:%M')}\n\nTap Auction House to bid now")

async def add_event_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID: return
    args = context.args
    if len(args) < 4:
        await update.message.reply_text("Usage: /add_event <title> <description> <date_YYYY-MM-DD> <location> [url]"); return
    title = args[0].replace("_"," "); description = args[1].replace("_"," ")
    try:
        from datetime import datetime
        event_date = datetime.strptime(args[2], "%Y-%m-%d")
    except:
        await update.message.reply_text("Date format: YYYY-MM-DD"); return
    location = args[3].replace("_"," "); ticket_url = args[4] if len(args) > 4 else None
    from database import get_db, release_db
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("INSERT INTO events (title, description, event_date, location, ticket_url) VALUES (%s,%s,%s,%s,%s) RETURNING id", (title, description, event_date, location, ticket_url))
        eid = cur.fetchone()[0]; conn.commit()
    finally:
        release_db(conn)
    await update.message.reply_text(f"EVENT ADDED\n\nID: {eid}\nTitle: {title}\nDate: {event_date.strftime('%d/%m/%Y')}\nLocation: {location}")
    msg = f"NEW EVENT ANNOUNCED\n\n{title}\n{description}\nDate: {event_date.strftime('%d %B %Y')}\nLocation: {location}"
    if ticket_url: msg += f"\n\nTickets: {ticket_url}"
...
async def panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    from telegram import InlineKeyboardMarkup, InlineKeyboardButton
    from config import SOFT_GATE, SERVICES, PARISH_LOUNGE, RADIO_CHANNEL_LINK, BOOKING_TERMS
    query = update.callback_query; await query.answer()
    ...
    if panel == "music":
        ...
        try:
            ...
            cur.execute("SELECT COUNT(*) FROM artist_submissions WHERE status='approved'"); total = cur.fetchone()[0]
        finally:
            release_db(conn)
        if not approved:
            await query.message.reply_text("ARTIST SPOTLIGHT\n\nNo featured artists yet.\n\nSubmit your music for review.\nTap Submit Track."); return
        ...
        from config import STREAK_BADGES, STREAK_BONUSES
        from database import get_db, release_db
...
async def action_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "booking_terms":
        ...
        from handlers.music import cart_view
...
async def ai_chat_handler_fn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        response = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "system", "content": AI_SYSTEM_PROMPT}, *history], max_tokens=400)
...
async def auction_bid_cb_fn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "auction_mybids":
        ...
        try:
            ...
            release_db(conn)
        if not bids:
            await query.message.reply_text("You have not placed any bids yet."); return
        text = "MY BIDS\n\n"
...
async def auction_bid_text_handler(uid, text, update, context):
    ...
    if bid_amount < session["min_bid"]:
        await update.message.reply_text(f"Bid too low. Minimum is {session['min_bid']:,} coins."); return True
    conn = get_db(); cur = conn.cursor()
    try:
        cur.execute("SELECT points FROM users WHERE telegram_id=%s", (uid,))
        fan = cur.fetchone()
        if not fan or fan[0] < bid_amount:
            await update.message.reply_text(f"Not enough coins. You have {fan[0] if fan else 0:,} coins."); return True
        ...
        auction = cur.fetchone()
        if not auction:
            await update.message.reply_text("This auction has ended."); return True
        old_top = auction[2]
        cur.execute("UPDATE auctions SET current_bid=%s, top_bidder=%s, top_username=%s WHERE id=%s", (bid_amount, uid, name, session["auction_id"]))
        cur.execute("INSERT INTO auction_bids (auction_id, telegram_id, username, amount) VALUES (%s,%s,%s,%s)", (session["auction_id"], uid, name, bid_amount))
        conn.commit()
    finally:
        release_db(conn)
    await update.message.reply_text(f"BID PLACED\n\nItem: {session['title']}\nYour bid: {bid_amount:,} coins\n\nYou are the top bidder.", reply_markup=main_menu)
    if old_top and old_top != uid:
        try:
            await context.bot.send_message(old_top, f"YOU HAVE BEEN OUTBID\n\nItem: {session['title']}\nNew top bid: {bid_amount:,} coins\n\nGo to Auction House to bid again.")
        except: pass
    return True
        ...
        try:
            ...
            try:
                ...
                for aid, title, winner_id, winner_name, final_bid in cur.fetchall():
                    cur.execute("UPDATE auctions SET status='ended' WHERE id=%s", (aid,)); conn.commit()
                    if winner_id:
                        try:
                            await telegram_app.bot.send_message(winner_id, f"YOU WON THE AUCTION\n\n{title}\nFinal bid: {final_bid:,} MiserCoins\n\nBAZRAGOD will deliver your item. Parish 14.")
                        except: pass
                        try:
                            await telegram_app.bot.send_message(OWNER_ID, f"AUCTION ENDED\n\n{title}\nWinner: @{winner_name} ({winner_id})\nFinal bid: {final_bid:,} coins")
                        except: pass
            finally:
                release_db(conn)
        except Exception as e:
...
async def send_weekly_intel():
    ...
    report = (
        ...
        f"TOP FANS\n{fans_text}\n\n"
        f"TOP SONGS\n{songs_text}\n\n"
        f"MAXIMUS INTEL SYSTEM v18.000"
    )
    try:
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in auction_bid_sessions:
        if await auction_bid_text_handler(uid, text, update, context): return
    ...
    if context.user_data.get("shop_dedication"):
        ...
        if deduct_points(uid, cost, "shop_dedication"):
            ...
            try:
                ...
                release_db(conn)
            await update.message.reply_text(f"DEDICATION QUEUED\n\nRef #{ded_id}\n\nMAXIMUS will announce your dedication on BazraGod Radio.\n\nParish 14 Nation.", reply_markup=main_menu)
        else:
            await update.message.reply_text("Not enough coins.")
        return
    if context.user_data.get("ai_active"):
        if await ai_chat_handler_fn(update, context): return
    ...
    routes = {
        "MUSIC SYSTEM": lambda u, c: panel_cb_fake(u, c, "music"),
        "STORE": store_panel,
        "COMMUNITY": lambda u, c: panel_cb_fake(u, c, "community"),
        "FAN ECONOMY": lambda u, c: panel_cb_fake(u, c, "economy"),
        "SOCIAL ACCESS": lambda u, c: panel_cb_fake(u, c, "social"),
...
async def panel_cb_fake(update, context, panel):
    from telegram import InlineKeyboardMarkup, InlineKeyboardButton
    ...
    update.callback_query.answer = asyncio.coroutine(lambda: None)
    class FakeQuery:
        def __init__(self):
            self.from_user = update.effective_user
            self.message = update.message
            self.data = f"panel:{panel}"
...
telegram_app.add_handler(CommandHandler("weekly", lambda u, c: asyncio.ensure_future(send_weekly_intel())))

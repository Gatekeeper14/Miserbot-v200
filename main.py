...
║  👥 Referral Milestones — tiered automatic rewards           ║
║     1  invite  → +20 pts instant                             ║
...
║  🗑️ Cache invalidation on every content change               ║
╚══════════════════════════════════════════════════════════════╝
“””
...
```
        if item["type"] == "ad":
            await telegram_app.bot.send_message(
                chat_id=channel_id,
                text=(f"📻 {dj['emoji']} BazraGod Radio — {now}\n\n"
...
if invites == 5:
    ...
    if beat:
        await context.bot.send_audio(uid, beat[1],
    ...
    if vault_item:
        ...
        await context.bot.send_audio(uid, vault_item[2],
...
if INTRO_FILE_ID:
await query.message.reply_text(f”{lang_name} {t(lang, ‘lang_saved’)}\n\n👑 Before you enter — Press play. Real fans only. 🎙️”)
await query.message.reply_voice(
INTRO_FILE_ID,
caption=“🎙️ BAZRAGOD — The Vision\nI.A.A.I.M.O”,
reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(“▶️  Enter The Platform”, callback_data=“intro:play”)]]),
)
else:
...
text += f”{medals[i]} {title}\n   {calculate_heat(likes, donations, plays)}  {plays:,} plays  ❤️{likes}  💰{donations}\n\n”
...
release_db(conn)
if not rows:
await update.message.reply_text(“No beats uploaded yet. Heat incoming 🎹”); return
...
release_db(conn)
if not rows:
await update.message.reply_text(“No drops yet. Stay tuned 🔥”); return
...
await update.message.reply_text(f”⚔️ BAZRAGOD CIPHER\n{‘═’*20}\n\nYou:\n{text}\n\nMAXIMUS:\n{verse}\n\n{‘═’*20}\n+{pts} pts 🔥”)
...
req_id = cur.fetchone()[0]; conn.commit()
finally:
release_db(conn)
...
await update.message.reply_text(f”🎙️ Submission #{sid} received!\nApproved voices play on BazraGod Radio. 🔥\n\n+{pts} pts 👑”, reply_markup=main_menu)
try:
...
# ║      🎚️ SMART AUDIO UPLOAD + FAN SUBMISSION ROUTING         ║
...
reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(“💵 Pay via CashApp”, url=CASHAPP)], [InlineKeyboardButton(“💳 Pay via PayPal”, url=PAYPAL)]]),
)
...
reply_markup=InlineKeyboardMarkup([[
InlineKeyboardButton(“🔗 Share Link”, url=f”https://t.me/share/url?url={link}&text=Join+Parish+14+Nation+on+BazraGod+Radio+%F0%9F%94%A5”)
]]),
)
...
f”#vault 2000  (vault at 2000 pts)\n”
...
f”🔄 Songs Aired:     {total_hist} total\n”
...
async def *delete_from(update, context, table):
args = context.args
if not args or not args[0].isdigit():
await update.message.reply_text(f”Usage: /delete*{table[:-1]} <id>”); return
row_id = int(args[0])
conn   = get_db()
cur    = conn.cursor()
try:
...
return (
f”I.A.A.I.M.O ONLINE — PARISH 14 NATION v15.000 | “
f”RADIO: {radio_status} | “
f”PLAYLIST: {len(playlist)} items (cached) | “
...
print(f"║   Songs:       {len(SEED_SONGS):<5} tracks                  ║")
print(f"║   Beats:       {len(SEED_BEATS):<5} beats                   ║")
print(f"║   Drops:       {len(SEED_DROPS):<5} drops                   ║")
print(f"║   Playlist:    {len(playlist):<5} items (cached)            ║")
print(f"║   Cache TTL:   {PLAYLIST_CACHE_TTL}s                       ║")
print(f"║   Channel:     {RADIO_CHANNEL_ID or 'NOT SET':<30}║")
print("║   Languages:   6 ACTIVE                      ║")
print("║   Vault:       ACTIVE                        ║")
print("║   Submissions: ACTIVE                        ║")
print("║   Anti-Repeat: ACTIVE (last 8 excluded)      ║")
print("║   Status:      ONLINE                        ║")
print("╚══════════════════════════════════════════════╝")

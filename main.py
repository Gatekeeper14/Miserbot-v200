...
║  Owner:   BAZRAGOD                                           ║
║                                                              ║
║  v15 VIRAL GROWTH ENGINE — ALL SYSTEMS CONNECTED:           ║
║  🔐 Secret Vault — point-gated exclusive content             ║
║  📤 Artist Submissions — fans pitch tracks to BAZRAGOD       ║
║  📡 Playlist Cache — 300s TTL, zero rebuild overhead         ║
║  📻 Radio History + Anti-Repeat Engine                        ║
║  🏆 Referral Milestone Rewards — tiered unlocks              ║
║     1 invite  → +50 pts                                      ║
║     5 invites → exclusive beat unlock                        ║
║     10 invites → vault song unlock                           ║
║     25 invites → 7-day Supporter FREE                        ║
║     50 invites → Nation Elite PERMANENT                      ║
...
AD_MESSAGES = [
    ...
    "📢 Drop your verse. Tap ⚔️ Lyric Cipher and battle MAXIMUS.",
...
# ╔══════════════════════════════════════════════════════════════╗
# ║         🎵 SEED DATA — SONGS / BEATS / DROPS / ANN           ║
# ╚══════════════════════════════════════════════════════════════╝
...
Platform lives inside Telegram. Catalog: 16 tracks. Nation: Parish 14.
...
# ╔══════════════════════════════════════════════════════════════╗
# ║             🌟 DUAL RANK SYSTEM                              ║
# ║  Nation Tier   → stored in DB, from RANKS thresholds         ║
# ║  Station Rank  → computed on the fly from LISTENER_RANKS     ║
# ╚══════════════════════════════════════════════════════════════╝
...
# Station Rank — displayed in profile alongside Nation Tier
LISTENER_RANKS = [
    (0,     "🎧 Listener"),
    (500,   "⚔️  Supporter"),
    (2000,  "🌟 Ambassador"),
...
def get_station_rank(points: int) -> str:
...
def get_next_station_rank_info(points: int) -> str:
    for threshold, label in LISTENER_RANKS:
        if points < threshold:
...
submission_sessions: dict = {}   # NEW v15 — artist submission flow

USER_PLAYLIST_INDEX: dict = {}
RADIO_LAST_ANNOUNCE: dict = {}

# ╔══════════════════════════════════════════════════════════════╗
# ║         📡 PLAYLIST CACHE — 300 SECOND TTL                  ║
# ╚══════════════════════════════════════════════════════════════╝

_PLAYLIST_CACHE:      list  = []
_PLAYLIST_LAST_BUILD: float = 0.0

def get_cached_playlist() -> list:
    """
    Returns the channel radio playlist.
    Rebuilds only if cache is older than PLAYLIST_CACHE_TTL seconds.
    Zero-overhead for 300 seconds after each build.
    """
    ...
    if time.time() - _PLAYLIST_LAST_BUILD > PLAYLIST_CACHE_TTL:
        ...
        print(f"📡 Playlist cache rebuilt — {len(_PLAYLIST_CACHE)} items")
    return _PLAYLIST_CACHE

def invalidate_playlist_cache():
    """Force cache rebuild on next call. Call after adding new content."""
...
def init_db():
    ...
    try:
        ...
        conn.commit()
        print("I.A.A.I.M.O DATABASE READY — v15.000")
        print(f"  Songs:              {len(SEED_SONGS)}")
        print(f"  Beats:              {len(SEED_BEATS)}")
        print(f"  Drops:              {len(SEED_DROPS)}")
        print(f"  Announcements:      {len(SEED_ANNOUNCEMENTS)}")
        print(f"  Vault:              ready")
        print(f"  Artist Submissions: ready")
        print(f"  Radio History:      ready")
    finally:
        release_db(conn)
...
# ╔══════════════════════════════════════════════════════════════╗
# ║  🎛️ PROFESSIONAL RADIO ROTATION ENGINE + ANTI-REPEAT        ║
# ║                                                              ║
# ║  Pattern per cycle:                                          ║
# ║  Song → Drop → Song → Drop → Beat →                         ║
# ║  Song → Drop → Song → AD_BREAK → Announcement → repeat      ║
# ║                                                              ║
# ║  Anti-repeat: excludes last 8 played songs from DB history   ║
# ╚══════════════════════════════════════════════════════════════╝

def build_professional_playlist() -> list:
    """
    Builds a full professional radio rotation.
    Songs are anti-repeat filtered against the last 8 played.
    Drops, beats, and announcements rotate through all available.
    """
    conn = get_db()
    cur  = conn.cursor()
    try:
        # Anti-repeat: exclude songs played in last 8 radio history entries
        cur.execute("""
        ...
        # Fallback: if all songs are in history, use all songs
        if not songs:
            cur.execute(
                "SELECT title, file_id, plays, likes, donations, rotation FROM songs ORDER BY id"
            )
            songs = cur.fetchall()
...
def _log_radio_history(file_id: str, title: str):
    """Log a played song to radio_history. Keep only last 50 entries."""
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute(
            "INSERT INTO radio_history (file_id, title) VALUES (%s, %s)",
            (file_id, title),
        )
        ...
        cur.execute("""
            ...
            WHERE id NOT IN (
                SELECT id FROM radio_history ORDER BY played_at DESC LIMIT 50
            )
        """)
        conn.commit()
    except Exception as e:
        print(f"Radio history log error: {e}")
    finally:
        release_db(conn)


def get_next_playlist_item(uid: int) -> dict:
    """Return next item for this user (in-chat radio) and advance their index."""
    playlist = build_professional_playlist()  # fresh for in-chat (no cache)
    if not playlist:
...
# ╔══════════════════════════════════════════════════════════════╗
# ║       📻 24/7 CHANNEL RADIO LOOP — CACHED + HISTORY         ║
# ╚══════════════════════════════════════════════════════════════╝
...
async def channel_radio_loop():
    ...
    while True:
        try:
            playlist = get_cached_playlist()  # Uses 300s cache
            if not playlist:
            ...
            if playlist_index >= len(playlist):
                playlist_index = 0
                # Invalidate cache at end of full cycle so new content can come in
            ...
            if item["type"] == "ad":
                ...
                await telegram_app.bot.send_message(
                    ...
                    text=(
                        ...
                        f"{ad_text}\n\n"
                        f"Join the movement: t.me/{BOT_USERNAME}"
                    ),
                )
            ...
            # ── SONG ─────────────────────────────────────────
                ...
                # Log to radio history for anti-repeat engine
...
def register_fan(telegram_id: int, username: str, referrer_id: int = None) -> bool:
    ...
    try:
        ...
        if cur.fetchone():
            return False
        cur.execute(
            "INSERT INTO fans (telegram_id, username, referrer_id) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING",
...
def get_listener_count() -> int:
    ...
    try:
        cur.execute(
            "SELECT COUNT(*) FROM radio_sessions WHERE last_ping > NOW() - INTERVAL '30 minutes'"
        )
...
# ╔══════════════════════════════════════════════════════════════╗
# ║     🏆 REFERRAL MILESTONE REWARDS — VIRAL GROWTH ENGINE     ║
# ║                                                              ║
# ║  1  invite  → +20 pts (instant, in award_points)            ║
# ║  5  invites → exclusive beat sent directly                  ║
# ║  10 invites → first vault item unlocked                     ║
# ║  25 invites → 7-day Supporter status FREE                   ║
# ║  50 invites → Nation Elite PERMANENT                        ║
# ╚══════════════════════════════════════════════════════════════╝

async def check_referral_milestones(uid: int, username: str, context):
    """Called every time a referrer earns an invite credit."""
    conn = get_db()
    cur  = conn.cursor()
    try:
    ...
    # ── 10 Invites → First Vault Item ──────────────────────────
        ...
        if vault_item:
            ...
            try:
                cur.execute(
                    "INSERT INTO vault_unlocks (telegram_id, vault_id) VALUES (%s, %s) ON CONFLICT DO NOTHING",
                    (uid, vault_item[0]),
                )
                conn.commit()
            finally:
    ...
    # ── 25 Invites → 7-Day Supporter FREE ─────────────────────
        ...
        try:
            cur.execute(
                "UPDATE fans SET is_supporter = TRUE, supporter_expires = %s WHERE telegram_id = %s",
                (expires, uid),
            )
            conn.commit()
        finally:
        ...
        await context.bot.send_message(
            ...
            f"7-day Parish 14 Supporter status.\n"
            f"Expires: {expires.strftime('%B %d, %Y')}\n\n"
            f"BAZRAGOD rewards those who build the army. 👑"
        )
    ...
    # ── 50 Invites → Nation Elite PERMANENT ───────────────────
        ...
        try:
            cur.execute(
                "UPDATE fans SET tier = '🌍  Nation Elite', is_supporter = TRUE WHERE telegram_id = %s",
                (uid,),
            )
            conn.commit()
        finally:
    ...
    if is_new and referrer:
        try:
            await context.bot.send_message(
                ...
                f"+{POINTS['invite_friend']} pts credited 🔥\n\n"
                f"Check your milestone rewards — 5, 10, 25, 50 invites unlock big prizes! 👑"
            )
...
# ╔══════════════════════════════════════════════════════════════╗
# ║     📻 IN-CHAT RADIO — PROFESSIONAL ROTATION + NEXT BUTTON  ║
# ╚══════════════════════════════════════════════════════════════╝
...
async def play_song(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT title, file_id, plays, likes, donations FROM songs WHERE id = %s", (song_id,)
        )
        song = cur.fetchone()
        if song:
...
async def like_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if song_id == 0:
        await query.answer("❤️ Tap a song from the catalog to like it!", show_alert=False); return
    uid  = query.from_user.id
    name = query.from_user.username or query.from_user.first_name
    conn = get_db()
    cur  = conn.cursor()
...
async def song_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for title, plays, likes, donations, rotation in songs:
        text += (
            f"🎵 {title}  {rb.get(rotation,'')}\n"
            f"Plays: {plays:,}  Likes: {likes}  Donations: {donations}\n"
            f"Heat: {calculate_heat(likes, donations, plays)}\n\n"
        )
    await update.message.reply_text(text)
...
async def vault_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not items:
        await update.message.reply_text(
            ...
            f"The vault is loading with exclusive content.\n\n"
            f"Earn more points to unlock when it opens. 👑\n\n"
    ...
    for vid, title, req_pts in items:
        if vid in unlocked or fan_points >= req_pts:
            ...
            btn_label = f"🔒 {title} — {req_pts:,} pts"
        keyboard.append([InlineKeyboardButton(btn_label, callback_data=f"vault:{vid}")])

    await update.message.reply_text(
        f"🔐 SECRET VAULT\n\n"
        f"Your Points: {fan_points:,}\n\n"
        f"Exclusive BAZRAGOD content.\n"
        f"Unlock with Parish 14 points. 👑\n\n"
        f"Select 👇",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def vault_item_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        fan_points = fan_row[0] if fan_row else 0
        cur.execute(
            "SELECT 1 FROM vault_unlocks WHERE telegram_id = %s AND vault_id = %s",
            (uid, vault_id),
        )
    ...
    if already_unlocked or fan_points >= req_pts:
        ...
        await query.answer(
            f"🔒 Need {needed:,} more points to unlock this.",
            show_alert=True,
        )
...
async def handle_artist_submission_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles audio sent by fans in submission_sessions."""
    uid  = update.effective_user.id
    name = get_username(update)
    ...
    # Parse "ArtistName - SongTitle" from caption
    ...
    try:
        cur.execute(
            """INSERT INTO artist_submissions (telegram_id, username, artist_name, song_title, file_id)
    ...
    await update.message.reply_text(
        ...
        f"Approved tracks go live on BazraGod Radio. 🔥\n\n"
        f"+{pts} pts 👑",
        reply_markup=main_menu,
    )
    try:
...
async def approve_submission_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "UPDATE artist_submissions SET status = 'approved' WHERE id = %s "
            "RETURNING telegram_id, artist_name, song_title, file_id",
            (sub_id,),
        )
        row = cur.fetchone()
        if row:
            tid, artist, song, file_id = row
            # Add to songs table so it plays on radio
            cur.execute(
                "INSERT INTO songs (title, file_id) VALUES (%s, %s) ON CONFLICT (title) DO NOTHING",
                (f"{artist} — {song}", file_id),
            )
            conn.commit()
            invalidate_playlist_cache()  # Force cache rebuild with new song
        conn.commit()
    finally:
        release_db(conn)
    ...
    if row:
        await update.message.reply_text(
            ...
            f"{artist} — {song} added to BazraGod Radio rotation. 📻"
        )
        try:
            await context.bot.send_message(
                row[0],
                f"🎙️ YOUR TRACK WAS APPROVED!\n\n"
                f"{row[1]} — {row[2]}\n\n"
                f"Now live on BazraGod Radio. 📻\n\n"
...
async def reject_submission_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "UPDATE artist_submissions SET status = 'rejected' WHERE id = %s RETURNING telegram_id",
            (sub_id,),
        )
        row = cur.fetchone()
        conn.commit()
...
async def list_submissions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT id, artist_name, song_title, status, submitted_at FROM artist_submissions "
    ...
    for r in rows:
        badge = "✅" if r[3] == "approved" else "❌" if r[3] == "rejected" else "⏳"
...
async def mood_radio_handler(uid: int, text: str, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        cur.execute(
            "SELECT id, title, file_id, plays, likes, donations FROM songs WHERE id = %s", (song_id,)
        )
        song = cur.fetchone()
        if not song:
            cur.execute(
...
async def voice_wall_submit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.forward_message(
            chat_id=OWNER_ID, from_chat_id=uid, message_id=update.message.message_id
        )
    except Exception:
        pass
    await maybe_prophecy(uid, name, context)
...
async def approve_voice_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row:
        ...
        try:
            await context.bot.send_message(
                row[0], "🎙️ YOUR VOICE WAS APPROVED!\nIt plays live on BazraGod Radio. 🔥\nParish 14. 👑"
            )
...
async def handle_audio_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Fan in submission session → route to submission handler ──
    if not is_admin(uid):
        if submission_sessions.get(uid):
            await handle_artist_submission_audio(update, context)
        return  # Non-admin non-submission audio → ignore

    # ── Admin audio upload flow ────────────────────────────────
    title   = audio.title or audio.file_name or (update.message.caption or "").strip() or "Untitled"
    file_id = audio.file_id
    caption = (update.message.caption or "").strip().lower()

    # Vault tag: #vault 2000 → adds to vault with 2000 required points
...
async def _save_vault_audio(msg, file_id: str, title: str, required_points: int):
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute(
            "INSERT INTO vault (title, file_id, required_points) VALUES (%s, %s, %s) RETURNING id",
            (title, file_id, required_points),
        )
        new_id = cur.fetchone()[0]; conn.commit()
    finally:
        release_db(conn)
    await msg.reply_text(
        f"🔐 Vault item added. ID: {new_id}\nTitle: {title}\nRequired: {required_points:,} pts"
    )
...
async def upload_classify_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _save_classified_audio(
        query.message, session["file_id"], session["title"], dest, label_map.get(dest, "✅")
    )
    invalidate_playlist_cache()
...
async def _save_classified_audio(msg, file_id: str, title: str, dest: str, label: str):
    ...
    try:
        if dest.startswith("dj:"):
            ...
            if cur.fetchone():
                text = f"⚠️ Song '{title}' already exists. Skipped."
            else:
                cur.execute(
                    "INSERT INTO songs (title, file_id) VALUES (%s, %s) RETURNING id", (title, file_id)
                )
                new_id = cur.fetchone()[0]; conn.commit()
                text = f"✅ {label} added. ID: {new_id}\nTitle: {title}"
        else:
            cur.execute(
                f"INSERT INTO {dest} (title, file_id) VALUES (%s, %s) RETURNING id", (title, file_id)
            )
            new_id = cur.fetchone()[0]; conn.commit()
            text = f"✅ {label} added. ID: {new_id}\nTitle: {title}"
    finally:
...
async def fan_radar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT COALESCE(country, 'Unknown'), COUNT(*) FROM fan_locations GROUP BY country ORDER BY 2 DESC LIMIT 10"
        )
        rows = cur.fetchall()
...
async def leaderboard_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if lb_type == "today":
            ...
            cur.execute("""
                ...
                ORDER BY pts DESC LIMIT 10
            """)
            label = "📅 THIS WEEK'S TOP FANS"
        else:
            cur.execute(
                "SELECT username, points, tier, is_supporter FROM fans ORDER BY points DESC LIMIT 10"
            )
    ...
    for i, (username, points, tier, is_sup) in enumerate(rows):
        ...
        text     += f"{medals[i]} {label_str}{badge}\n   {points:,} pts — {tier}\n\n"
...
async def my_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            ...
            "is_supporter, supporter_expires, language FROM fans WHERE telegram_id = %s",
            (uid,),
        )
        row = cur.fetchone()
        cur.execute(
    ...
    await update.message.reply_text(
        ...
        f"Station Rank: {station}\n"
        f"Points:       {points:,}\n"
        f"Global Rank:  #{rank_pos}\n"
        f"Invites:      {invites}\n"
        f"Vault Items:  {vault_count} unlocked\n"
        f"City:         {location}\n"
...
async def send_weekly_intel():
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM radio_history WHERE played_at > NOW() - INTERVAL '7 days'"); songs_this_week = cur.fetchone()[0]
    finally:
        release_db(conn)
    ...
    report = (
        ...
        f"🥁 Beats in Library:    {total_beats}\n"
        f"🎤 Drops in Library:    {total_drops}\n"
        f"📻 Songs Aired:         {songs_this_week} (this week)\n\n"
        f"TOP COUNTRIES:\n{countries_text}\n\n"
        f"TOP FANS:\n{fans_text}\n\n"
        f"TOP SONGS:\n{songs_text}\n\n"
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rotation_preview = "→".join([
        {"song": "🎵", "drop": "🎤", "beat": "🥁", "ad": "📢", "announcement": "📣"}.get(p["type"], "?")
        for p in playlist[:12]
    ...
    await update.message.reply_text(
        ...
        f"SUBMISSIONS\n"
        f"/list_submissions\n"
        f"/approve_submission <id>\n"
        f"/reject_submission <id>\n\n"
        f"PREMIERE\n/premiere <song_id>\n\n"
        f"RADIO\n/start_radio\n\n"
        f"VOICE WALL\n"
        f"/list_voices\n/approve_voice <id>\n/reject_voice <id>\n\n"
        ...
        f"Caption: #song #beat #drop #promo #announce\n"
...
async def start_radio_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Broadcasting to: {RADIO_CHANNEL_ID}\n"
        f"Playlist items: {len(playlist)}\n"
        f"Cache TTL: {PLAYLIST_CACHE_TTL}s\n"
        f"Anti-repeat: Last 8 songs excluded\n"
        f"Pattern: Song→Drop→Beat→Song→Drop→Beat→AD→Announce\n\n"
        f"Station is live. Parish 14 Nation. 👑"
    )
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM radio_history"); total_history = cur.fetchone()[0]
    finally:
        release_db(conn)
    ...
    await update.message.reply_text(
        ...
        f"📡 Channel Radio:   {'ACTIVE ✅' if radio_loop_running else 'STANDBY'}\n"
        f"🎛️ Playlist Items:  {len(playlist)} (cached)\n"
        f"🔄 Songs Aired:     {total_history} total\n"
...
async def radar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT COALESCE(country,'Unknown'), COUNT(*) FROM fan_locations GROUP BY country ORDER BY 2 DESC LIMIT 15"
        )
        rows = cur.fetchall()
...
async def orders_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT id, telegram_id, item, price, purchased_at FROM purchases WHERE status = 'pending' ORDER BY purchased_at DESC LIMIT 20"
        )
        rows = cur.fetchall()
    finally:
        release_db(conn)
...
async def premiere_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    invalidate_playlist_cache()
    await update.message.reply_text(
        f"🎙️ Premiere broadcast sent to {sent} fans.\nSong '{title}' set to 🔴 A rotation."
    )
...
async def list_requests_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cur.execute(
            "SELECT id, username, song_title, played, requested_at FROM song_requests ORDER BY id DESC LIMIT 20"
        )
        rows = cur.fetchall()
    finally:
        release_db(conn)
    if not rows:
        await update.message.reply_text("No requests yet."); return
    text = "📀 SONG REQUESTS\n\n"
    for r in rows:
...
async def list_dj_drops_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        release_db(conn)
    if not rows:
        await update.message.reply_text("No DJ drops yet."); return
    text = "🎙️ DJ DROPS\n\n"
    for r in rows:
        text += f"[{r[0]}] {r[1]} — {r[2]}\n"
    await update.message.reply_text(text)
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    for d in [pending_broadcasts, astro_sessions, mood_sessions, cipher_sessions,
              upload_sessions, request_sessions, submission_sessions]:
        d.pop(uid, None)
...
async def menu_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    for d in [astro_sessions, mood_sessions, cipher_sessions, upload_sessions,
              request_sessions, submission_sessions]:
        d.pop(uid, None)
    context.user_data["ai_active"]         = False
...
async def list_songs_cmd(update, context):
    ...
    for r in rows:
        text += f"[{r[0]}] {rb.get(r[5],'')} {r[1]}\n   {calculate_heat(r[3],r[4],r[2])} {r[2]:,} plays  ❤️{r[3]}  💰{r[4]}\n"

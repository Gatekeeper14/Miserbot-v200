...
async def radio(update,context):
    ...
    if song:
        ...
        dj_lines=[
        f"You are now listening to BazraGod Radio. The time is {now}. Next up {song[0]}",
        f"This is I.A.A.I.M.O radio. Playing {song[0]}.",
        f"No label. No middleman. BazraGod Radio presents {song[0]}"
        ]

        await maximus_speak(context,update.effective_user.id,random.choice(dj_lines))
...
async def leaderboard(update,context):
    await update.message.reply_text("Leaderboard system initializing...")
...
async def location_prompt(update,context):
    ...
    await update.message.reply_text(
        "📍 Share your location to join the BAZRAGOD fan map.",
        reply_markup=keyboard
    )
...
async def location_handler(update,context):
    ...
    """,
    (user.id,loc.latitude,loc.longitude,loc.latitude,loc.longitude))

    conn.commit()
    conn.close()

    await update.message.reply_text("🌍 Location saved. You are on the fan map.")
...
async def upload(update,context):
    ...
    await update.message.reply_text("Song uploaded")

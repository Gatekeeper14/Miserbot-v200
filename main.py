"""
╔══════════════════════════════════════════════════════════════╗
║         I.A.A.I.M.O — MASTER SYSTEM v16.000                 ║
║  Independent Artists Artificial Intelligence Music Ops       ║
║  Bot:     Miserbot       Nation:  Parish 14                  ║
║  Owner:   BAZRAGOD                                           ║
║                                                              ║
║  v16 SOVEREIGN ECONOMY ENGINE:                               ║
║  💎 $PARISH Point Economy — full ledger with categories      ║
║  🏛️  Auction House — timed bids, auto-notify, admin creates  ║
║  📦 NFT Drop System — limited drops, point bidding           ║
║  🔒 Fan Staking — lock points, earn multipliers              ║
║  🛍️  Point Shop — spend points on real rewards               ║
║  👑 Tier Dividends — Nation Elite daily point drip           ║
║  📻 Radio NOW PLAYING screen                                 ║
║  🔥 Listener Streaks — 3/7/30 day streaks + badges          ║
║  🎙️  Radio Dedications — spend points, MAXIMUS says name     ║
║  📡 Premium Supporter Radio channel                          ║
║  🗺️  Live Listener Map                                       ║
╚══════════════════════════════════════════════════════════════╝
"""
...
TRANSLATIONS = {
    "en": {
        ...
        "location_saved": "Location recorded! Your city is on the map. BAZRAGOD sees where his army stands.",
    ...
    "es": {
        ...
        "location_saved": "Ubicacion registrada! Tu ciudad esta en el mapa.",
        "mission_done":   "MISION COMPLETADA! Vuelve manana. Parish 14 nunca para.",
    },
    "fr": {
        ...
        "location_saved": "Localisation enregistree! Votre ville est sur la carte.",
        "mission_done":   "MISSION ACCOMPLIE! Reviens demain. Parish 14 ne s'arrete jamais.",
    },
    "pt": {
        ...
        "location_saved": "Localizacao registrada! Sua cidade esta no mapa.",
        "mission_done":   "MISSAO COMPLETA! Volte amanha. Parish 14 nunca para.",
    },
    "de": {
        ...
        "location_saved": "Standort gespeichert! Deine Stadt ist auf der Karte.",
        "mission_done":   "MISSION ABGESCHLOSSEN! Komm morgen wieder.",
    },
    "jm": {
...
DJS = {
    "aurora": {
        ...
        "intros": ["Good morning Parish 14. DJ Aurora on the frequency. Lets rise.", "The sun is up. BazraGod Radio morning session. DJ Aurora in control."],
    },
    "colorred": {
        ...
        "intros": ["Afternoon session live. DJ Color Red on the dial. Turn it up.", "Midday heat. BazraGod Radio. DJ Color Red taking no prisoners."],
    },
    "maximus": {
        ...
        "intros": ["Prime time. DJ Maximus commanding the airwaves. Parish 14 Nation.", "The sovereign hour begins. DJ Maximus. BazraGod Radio."],
    },
    "eclipse": {
        ...
        "intros": ["Late night. DJ Eclipse in the dark. BazraGod Radio never sleeps.", "Past midnight. The real ones are awake. DJ Eclipse."],
    },
}
...
AD_MESSAGES = [
    ...
    "Become a Supporter for $19.99 per month. Tap Supporter.",
    ...
    "Spend your PARISH points in the Point Shop. Tap Shop.",
    "Bid on the latest NFT drop. Tap Auction House.",
    "Stake your points and earn multipliers. Tap Staking.",
]
...
POINTS = {
    ...
    "vault_unlock": 5, "submit_track": 10, "streak_bonus": 0,
    "dedication": 0, "shop_purchase": 0, "auction_bid": 0,
}

REFERRAL_TIERS = {5: "Supporter", 10: "Recruiter", 25: "Commander", 50: "General"}
...
POINT_SHOP_ITEMS = {
    ...
    "vault_pass":    ("Vault Pass — unlock any one vault item",            200),
    "supporter_day": ("1-Day Supporter Trial",                             150),
    "shoutout":      ("Personal Shoutout from BAZRAGOD broadcast",         500),
    "exclusive_dm":  ("Exclusive DM — MAXIMUS personal message",           100),
}
...
MISSIONS = [
    ...
    "Stake your points for bonus multipliers",
    "Buy something from the Point Shop",
]
...
def init_db():
    ...
    try:
        ...
        for tbl in [
            ...
            "CREATE TABLE IF NOT EXISTS point_log (id SERIAL PRIMARY KEY, telegram_id BIGINT, action TEXT, pts INT, category TEXT DEFAULT 'earn', logged_at TIMESTAMP DEFAULT NOW())",
            ...
            "CREATE TABLE IF NOT EXISTS radio_sessions (telegram_id BIGINT PRIMARY KEY, joined_at TIMESTAMP DEFAULT NOW(), last_ping TIMESTAMP DEFAULT NOW(), streak_days INT DEFAULT 0, last_streak_date DATE)",
            ...
            """CREATE TABLE IF NOT EXISTS auctions (
                ...
                description  TEXT,
                file_id      TEXT,
                item_type    TEXT DEFAULT 'nft',
                starting_bid INTEGER DEFAULT 100,
            ...
            """CREATE TABLE IF NOT EXISTS nft_drops (
                ...
                editions_left INTEGER DEFAULT 1,
                price_pts    INTEGER DEFAULT 500,
                status       TEXT DEFAULT 'active',
                created_at   TIMESTAMP DEFAULT NOW()
            )""",
        ...
        for col, defn in [
            ...
            cur.execute(f"ALTER TABLE radio_sessions ADD COLUMN IF NOT EXISTS {col} {defn}")
        cur.execute("ALTER TABLE point_log ADD COLUMN IF NOT EXISTS category TEXT DEFAULT 'earn'")
...
async def channel_radio_loop():
    ...
    while True:
        try:
            ...
            pending_ded = _get_pending_dedication()
            if pending_ded and random.random() < 0.15:
                ded_text = f"This next track goes out to {pending_ded['username']} on BazraGod Radio. {pending_ded['message']}. Parish 14 Nation."
                await telegram_app.bot.send_message(chat_id=channel_id, text=f"Dedication to {pending_ded['username']}\n\n{pending_ded['message']}\n\nBazraGod Radio {now}")

            if item["type"] == "ad":
                await telegram_app.bot.send_message(chat_id=channel_id, text=f"BazraGod Radio {now}\n\n{random.choice(AD_MESSAGES)}\n\nt.me/{BOT_USERNAME}")
                await asyncio.sleep(RADIO_AD_DELAY)
            elif item["type"] == "announcement" and item["file_id"]:
                await telegram_app.bot.send_audio(chat_id=channel_id, audio=item["file_id"], caption=f"{item['title']}\nBazraGod Radio {now}\n\nt.me/{BOT_USERNAME}")
                await asyncio.sleep(RADIO_ANNOUNCE_DELAY)
            elif item["type"] == "drop" and item["file_id"]:
                await telegram_app.bot.send_audio(chat_id=channel_id, audio=item["file_id"], caption=f"{dj['name']}\nBazraGod Radio {now}\n\nt.me/{BOT_USERNAME}")
                await asyncio.sleep(RADIO_DROP_DELAY)
            elif item["type"] == "beat" and item["file_id"]:
                await telegram_app.bot.send_audio(chat_id=channel_id, audio=item["file_id"], caption=f"BAZRAGOD BEAT {item['title']}\nBazraGod Radio {now}\n\nt.me/{BOT_USERNAME}")
...
def get_multiplier(uid):
    ...
    try:
        cur.execute("""
            SELECT multiplier FROM stakes
            WHERE telegram_id = %s AND status = 'active' AND ends_at > NOW()
...
def award_points(telegram_id, action, username=None, category="earn"):
    base_pts = POINTS.get(action, 1)
    if base_pts == 0:
        return 0
    multiplier = get_multiplier(telegram_id)
    ...
    try:
        ...
        cur.execute("INSERT INTO point_log (telegram_id, action, pts, category) VALUES (%s, %s, %s, %s)", (telegram_id, action, pts, category))
...
def deduct_points(telegram_id, amount, action="spend"):
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("SELECT points FROM fans WHERE telegram_id = %s", (telegram_id,))
        ...
        if not row or row[0] < amount:
            return False
        cur.execute("UPDATE fans SET points = points - %s WHERE telegram_id = %s", (amount, telegram_id))
        cur.execute("INSERT INTO point_log (telegram_id, action, pts, category) VALUES (%s, %s, %s, 'spend')", (telegram_id, action, -amount))
        conn.commit()
        return True
    finally:
        release_db(conn)
...
def check_supporter_expiry():
    ...
    try:
        cur.execute("""
            ...
            WHERE is_supporter = TRUE AND supporter_expires IS NOT NULL AND supporter_expires < CURRENT_DATE
        """)
        conn.commit()
    finally:
        release_db(conn)
...
def update_listener(uid):
    ...
    try:
        today = date.today()
        cur.execute("SELECT last_streak_date, streak_days FROM radio_sessions WHERE telegram_id = %s", (uid,))
        row = cur.fetchone()
        if row:
            ...
            if last_date == today - timedelta(days=1):
                ...
                streak = 1
            cur.execute("""
                UPDATE radio_sessions SET last_ping = NOW(), last_streak_date = %s, streak_days = %s
                WHERE telegram_id = %s
            """, (today, streak, uid))
        else:
            cur.execute("""
                INSERT INTO radio_sessions (telegram_id, joined_at, last_ping, last_streak_date, streak_days)
                VALUES (%s, NOW(), NOW(), %s, 1)
            """, (uid, today))
        conn.commit()
        return streak if row else 1
    finally:
        release_db(conn)
...
def get_fan_streak(uid):
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("SELECT streak_days, last_streak_date FROM radio_sessions WHERE telegram_id = %s", (uid,))
        row = cur.fetchone()
        if not row:
            return 0
...
async def auction_house(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        fan = cur.fetchone()
        pts = fan[0] if fan else 0
        cur.execute("SELECT id, title, description, current_bid, top_username, ends_at, item_type FROM auctions WHERE status = 'active' AND ends_at > NOW() ORDER BY ends_at")
    ...
    if not auctions:
        await update.message.reply_text(
            "AUCTION HOUSE\n\nNo active auctions right now.\n\nBAZRAGOD drops new auctions for:\nExclusive songs\nNFT digital art\nFeature slots\nPersonal video calls\n\nStay tuned. Next drop incoming. 👑\n\nYour balance: " + f"{pts:,} PARISH pts"
        )
        return

    text = f"AUCTION HOUSE\n\nYour balance: {pts:,} PARISH pts\n\nActive auctions:\n\n"
    keyboard = []
    for auction in auctions:
        aid, title, desc, current_bid, top_user, ends_at, item_type = auction
        time_left = ends_at - datetime.now()
        hours_left = int(time_left.total_seconds() // 3600)
        mins_left  = int((time_left.total_seconds() % 3600) // 60)
        type_emoji = {"nft": "🎨", "feature": "🎤", "song": "🎵", "call": "📞", "merch": "👕"}.get(item_type, "💎")
        text += f"{type_emoji} {title}\nCurrent bid: {current_bid:,} pts\nTop bidder: {top_user or 'None yet'}\nEnds in: {hours_left}h {mins_left}m\n\n"
...
async def auction_bid_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fan_pts < min_bid:
        await query.answer(f"You need {min_bid:,} pts to bid. You have {fan_pts:,}.", show_alert=True)
        return

    auction_bid_sessions[uid] = {"auction_id": aid, "title": title, "min_bid": min_bid, "current_bid": current_bid}
    await query.message.reply_text(
        f"PLACE YOUR BID\n\nItem: {title}\nCurrent bid: {current_bid:,} pts\nMin bid: {min_bid:,} pts\nYour balance: {fan_pts:,} pts\n\nType your bid amount (numbers only).\nType /cancel to go back."
    )

async def _show_my_bids(query, uid):
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("""
            SELECT a.title, ab.amount, a.current_bid, a.top_bidder, a.ends_at
            FROM auction_bids ab
            JOIN auctions a ON ab.auction_id = a.id
            WHERE ab.telegram_id = %s
...
async def auction_bid_handler(uid, text, update, context):
    ...
    try:
        ...
        await update.message.reply_text("Invalid amount. Numbers only. Try again.")
        auction_bid_sessions[uid] = session
        return True

    if bid_amount < session["min_bid"]:
        await update.message.reply_text(f"Bid too low. Minimum is {session['min_bid']:,} pts.")
        return True
    ...
    try:
        ...
        if not fan or fan[0] < bid_amount:
            await update.message.reply_text(f"Not enough points. You have {fan[0] if fan else 0:,} pts.")
            return True
    ...
    await update.message.reply_text(
        f"BID PLACED!\n\nItem: {session['title']}\nYour bid: {bid_amount:,} pts\n\nYou are currently the top bidder. You will be notified if outbid. 👑",
        reply_markup=main_menu,
    )

    if old_top_bidder and old_top_bidder != uid:
        try:
            await context.bot.send_message(old_top_bidder,
                f"YOU HAVE BEEN OUTBID!\n\nItem: {session['title']}\nNew top bid: {bid_amount:,} pts\n\nGo back to the Auction House to bid again.")
        except Exception:
            pass

    return True

async def create_auction_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    args = context.args
    if len(args) < 4:
        await update.message.reply_text(
            "Usage: /create_auction <title> <description> <starting_bid> <hours>\n\n"
            "Example: /create_auction ExclusiveStem RareUnreleasedStem 200 48\n\n"
            "Then send an audio/image file with caption #auction to attach media."
        )
        return
    title        = args[0].replace("_", " ")
    description  = args[1].replace("_", " ")
    starting_bid = int(args[2])
    hours        = int(args[3])
    ...
    try:
        cur.execute("""
            INSERT INTO auctions (title, description, starting_bid, current_bid, ends_at)
            VALUES (%s, %s, %s, %s, %s) RETURNING id
        """, (title, description, starting_bid, starting_bid, ends_at))
        aid = cur.fetchone()[0]
        conn.commit()
    finally:
        release_db(conn)
    await update.message.reply_text(
        f"AUCTION CREATED!\n\nID: {aid}\nTitle: {title}\nStarting bid: {starting_bid:,} pts\nEnds: {ends_at.strftime('%d/%m %H:%M')}\n\nBroadcasting to all fans now..."
    )
    msg = f"NEW AUCTION LIVE!\n\nBAZRAGOD just dropped a new auction.\n\n{title}\n{description}\nStarting bid: {starting_bid:,} PARISH pts\nEnds: {ends_at.strftime('%d/%m/%Y %H:%M')}\n\nTap Auction House to bid now!"
...
async def nft_drops(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid  = update.effective_user.id
    conn = get_db()
    cur  = conn.cursor()
    try:
        ...
        fan = cur.fetchone()
        pts = fan[0] if fan else 0
        cur.execute("SELECT id, title, description, editions_left, price_pts FROM nft_drops WHERE status = 'active' AND editions_left > 0 ORDER BY created_at DESC")
        drops = cur.fetchall()
        cur.execute("SELECT nd.title FROM nft_ownership no2 JOIN nft_drops nd ON no2.nft_id = nd.id WHERE no2.telegram_id = %s", (uid,))
    ...
    if not drops:
        await update.message.reply_text(
            f"NFT DROPS\n\nYour balance: {pts:,} PARISH pts\n\nNo drops available right now.\n\nBAZRAGOD drops exclusive digital collectibles here. Follow the socials so you never miss a drop. 👑\n\nYou own: {len(owned)} items"
        )
        return

    text     = f"NFT DROPS\n\nYour balance: {pts:,} PARISH pts\n\nCollect exclusive BAZRAGOD digital items.\n\nAvailable now:\n\n"
    keyboard = []
    for nft_id, title, desc, editions_left, price_pts in drops:
        already_owned = title in owned
        text += f"🎨 {title}\n{desc}\n{editions_left} editions left\nPrice: {price_pts:,} pts\n{'OWNED' if already_owned else ''}\n\n"
        if not already_owned:
            keyboard.append([InlineKeyboardButton(f"Collect {title} — {price_pts:,} pts", callback_data=f"nft_buy:{nft_id}")])

    if owned:
        keyboard.append([InlineKeyboardButton(f"My Collection ({len(owned)} items)", callback_data="nft_collection")])

    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard) if keyboard else None)

async def nft_buy_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    uid  = query.from_user.id
    name = query.from_user.username or query.from_user.first_name or str(uid)

    if query.data == "nft_collection":
        await _show_nft_collection(query, uid)
        return

    try:
        nft_id = int(query.data.split(":")[1])
    except Exception:
        return
    ...
    cur  = conn.cursor()
    try:
        cur.execute("SELECT title, price_pts, editions_left FROM nft_drops WHERE id = %s AND status = 'active'", (nft_id,))
        nft = cur.fetchone()
        if not nft:
            await query.answer("This drop is no longer available.", show_alert=True)
            return
        title, price_pts, editions_left = nft
        if editions_left <= 0:
            await query.answer("Sold out!", show_alert=True)
            return
        cur.execute("SELECT 1 FROM nft_ownership WHERE nft_id = %s AND telegram_id = %s", (nft_id, uid))
        if cur.fetchone():
    ...
    if not deduct_points(uid, price_pts, "nft_purchase"):
        await query.answer(f"Not enough points. Need {price_pts:,}.", show_alert=True)
        return

    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("UPDATE nft_drops SET editions_left = editions_left - 1 WHERE id = %s", (nft_id,))
        cur.execute("UPDATE nft_drops SET status = 'sold_out' WHERE id = %s AND editions_left <= 0", (nft_id,))
        cur.execute("INSERT INTO nft_ownership (nft_id, telegram_id, username) VALUES (%s, %s, %s)", (nft_id, uid, name))
        conn.commit()
    finally:
        release_db(conn)

    await query.message.reply_text(
        f"NFT COLLECTED!\n\n{title}\n\nThis is yours. A piece of BAZRAGOD history.\nParish 14 Nation. 👑\n\nView your collection in NFT Drops."
    )
    try:
        await context.bot.send_message(OWNER_ID, f"NFT SOLD\n\n{title}\nBuyer: @{name} ({uid})\nPrice: {price_pts:,} pts")
    except Exception:
        pass

async def _show_nft_collection(query, uid):
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("""
            SELECT nd.title, nd.description, no2.acquired_at
            FROM nft_ownership no2 JOIN nft_drops nd ON no2.nft_id = nd.id
            WHERE no2.telegram_id = %s ORDER BY no2.acquired_at DESC
        """, (uid,))
        items = cur.fetchall()
    finally:
        release_db(conn)
    if not items:
        await query.message.reply_text("Your collection is empty. Go collect some NFT drops!")
        return
    text = f"MY NFT COLLECTION\n\n{len(items)} items\n\n"
    for title, desc, acquired_at in items:
        text += f"🎨 {title}\n{desc}\nAcquired: {acquired_at.strftime('%d/%m/%Y')}\n\n"
    await query.message.reply_text(text)

async def create_nft_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    args = context.args
    if len(args) < 4:
        await update.message.reply_text(
            "Usage: /create_nft <title> <description> <editions> <price_pts>\n\n"
            "Example: /create_nft ExclusiveCover RareAlbumCover 10 500"
        )
        return
    title       = args[0].replace("_", " ")
    description = args[1].replace("_", " ")
    editions    = int(args[2])
    price_pts   = int(args[3])
    conn        = get_db()
    cur         = conn.cursor()
    try:
        cur.execute("INSERT INTO nft_drops (title, description, edition_size, editions_left, price_pts) VALUES (%s, %s, %s, %s, %s) RETURNING id",
                    (title, description, editions, editions, price_pts))
        nft_id = cur.fetchone()[0]
        conn.commit()
    finally:
        release_db(conn)
    await update.message.reply_text(f"NFT DROP CREATED!\n\nID: {nft_id}\nTitle: {title}\nEditions: {editions}\nPrice: {price_pts:,} pts")
    msg = f"NEW NFT DROP!\n\nBAZRAGOD just dropped an exclusive.\n\n{title}\n{description}\n{editions} editions available\nPrice: {price_pts:,} PARISH pts\n\nTap NFT Drops to collect now!"
    await _broadcast_to_all(context, msg)
...
async def staking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        fan = cur.fetchone()
        pts = fan[0] if fan else 0
        cur.execute("SELECT amount, days, multiplier, started_at, ends_at, earned FROM stakes WHERE telegram_id = %s AND status = 'active' ORDER BY started_at DESC", (uid,))
        active_stakes = cur.fetchall()
    finally:
        release_db(conn)
    ...
    if active_stakes:
        stake_info = "\n\nACTIVE STAKES:\n"
        for amount, days, multiplier, started_at, ends_at, earned in active_stakes:
            time_left = ends_at - datetime.now()
            days_left = max(0, time_left.days)
            stake_info += f"Staked: {amount:,} pts | {days}d | {multiplier}x\n{days_left} days remaining | +{earned} earned\n"

    tier_text = "\n".join([f"{days} days = {mult}x multiplier on all earns" for days, mult in STAKE_TIERS.items()])

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("Stake 100 pts for 7 days (1.2x)", callback_data="stake:100:7")],
        [InlineKeyboardButton("Stake 250 pts for 14 days (1.35x)", callback_data="stake:250:14")],
        [InlineKeyboardButton("Stake 500 pts for 30 days (1.5x)", callback_data="stake:500:30")],
        [InlineKeyboardButton("Custom Amount", callback_data="stake:custom")],
    ])
    await update.message.reply_text(
        f"POINT STAKING\n\nLock your PARISH points and earn multipliers on everything you do.\n\nYour balance: {pts:,} pts\n\nSTAKE TIERS:\n{tier_text}{stake_info}\n\nStaked points are locked until the period ends.",
...
async def stake_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "stake:custom":
        ...
        await query.message.reply_text("Enter how many points to stake and how many days.\n\nFormat: amount days\nExample: 750 30\n\nMinimum: 50 points, 7 days")
        return

    try:
        _, amount_str, days_str = data.split(":")
        amount = int(amount_str)
        days   = int(days_str)
    except Exception:
        return

    await _process_stake(query.message, uid, name, amount, days, context)

async def stake_custom_handler(uid, text, update, context):
    if not context.user_data.get("staking_custom"):
        return False
    ...
    if days not in STAKE_TIERS:
        closest = min(STAKE_TIERS.keys(), key=lambda d: abs(d - days))
        await update.message.reply_text(f"Invalid stake period. Use 7, 14, or 30 days. Closest: {closest} days.")
        return True

    name = get_username(update)
    await _process_stake(update.message, uid, name, amount, days, context)
    return True

async def _process_stake(msg, uid, name, amount, days, context):
    if amount < 50:
        await msg.reply_text("Minimum stake is 50 points.")
        return
    if days not in STAKE_TIERS:
        await msg.reply_text("Stake period must be 7, 14, or 30 days.")
        return
    ...
    if not deduct_points(uid, amount, "stake_lock"):
        ...
        try:
            ...
            release_db(conn)
        await msg.reply_text(f"Not enough points to stake. You have {pts:,} pts. You need {amount:,} pts.")
        return
    ...
    cur     = conn.cursor()
    try:
        cur.execute("INSERT INTO stakes (telegram_id, amount, days, multiplier, ends_at) VALUES (%s, %s, %s, %s, %s) RETURNING id",
                    (uid, amount, days, multiplier, ends_at))
    ...
    await msg.reply_text(
        f"POINTS STAKED!\n\nAmount: {amount:,} PARISH pts\nPeriod: {days} days\nMultiplier: {multiplier}x on ALL earns\nUnlocks: {ends_at.strftime('%d/%m/%Y')}\n\nEvery point you earn from now is multiplied by {multiplier}x. 👑",
        reply_markup=main_menu,
    )

async def process_stake_maturity():
    while True:
        try:
            conn = get_db()
            cur  = conn.cursor()
            try:
                ...
                for sid, uid, amount, multiplier, days in matured:
                    bonus = int(amount * (multiplier - 1.0) * days * 0.5)
                    cur.execute("UPDATE stakes SET status = 'matured', earned = %s WHERE id = %s", (bonus, sid))
                    cur.execute("UPDATE fans SET points = points + %s WHERE telegram_id = %s", (amount + bonus, uid))
                    cur.execute("INSERT INTO point_log (telegram_id, action, pts, category) VALUES (%s, 'stake_return', %s, 'earn')", (uid, amount + bonus))
                    conn.commit()
                    try:
                        await telegram_app.bot.send_message(uid,
                            f"STAKE MATURED!\n\nYour {amount:,} pt stake has unlocked.\nBonus earned: +{bonus} pts\nTotal returned: {amount + bonus:,} pts\n\nParish 14 pays its soldiers. 👑")
                    except Exception:
                        pass
            finally:
                release_db(conn)
        except Exception as e:
            print(f"Stake maturity error: {e}")
...
async def point_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid  = update.effective_user.id
    conn = get_db()
    cur  = conn.cursor()
    try:
    ...
    text = f"POINT SHOP\n\nSpend your PARISH points on real rewards.\n\nYour balance: {pts:,} pts\n\n"
    keyboard = []
    for item_key, (item_name, cost) in POINT_SHOP_ITEMS.items():
        affordable = "✅" if pts >= cost else "🔒"
        text      += f"{affordable} {item_name}\nCost: {cost:,} pts\n\n"
        keyboard.append([InlineKeyboardButton(f"{affordable} {item_name} ({cost:,} pts)", callback_data=f"shop:{item_key}")])
...
async def shop_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    if item_key not in POINT_SHOP_ITEMS:
        return

    item_name, cost = POINT_SHOP_ITEMS[item_key]
    ...
    if pts < cost:
        await query.answer(f"Not enough points. Need {cost:,}, you have {pts:,}.", show_alert=True)
        return

    if item_key == "dedication":
        context.user_data["shop_dedication"] = True
        await query.message.reply_text(f"RADIO DEDICATION\n\nCost: {cost:,} pts\n\nType your dedication message.\nMAXIMUS will announce it on BazraGod Radio.\n\nExample: This one goes out to my sister Sarah in London. Keep grinding.\n\nType your message now.")
        return

    if not deduct_points(uid, cost, f"shop_{item_key}"):
        await query.answer("Purchase failed. Try again.", show_alert=True)
        return

    if item_key == "vault_pass":
        conn = get_db()
        cur  = conn.cursor()
        try:
            cur.execute("SELECT id FROM vault ORDER BY required_points LIMIT 1")
            vault_item = cur.fetchone()
        finally:
            release_db(conn)
        await query.message.reply_text(
            f"VAULT PASS PURCHASED!\n\nYou can now unlock any one vault item for free.\nGo to Secret Vault to redeem. 👑"
        )

    elif item_key == "supporter_day":
        expires = date.today() + timedelta(days=1)
        conn = get_db()
        cur  = conn.cursor()
        try:
            cur.execute("UPDATE fans SET is_supporter = TRUE, supporter_expires = %s WHERE telegram_id = %s", (expires, uid))
            conn.commit()
        finally:
            release_db(conn)
        await query.message.reply_text(f"1-DAY SUPPORTER ACTIVATED!\n\nYou are now a Parish 14 Supporter.\nExpires: {expires.strftime('%d/%m/%Y')}\n\nAll supporter benefits active now. 👑")

    elif item_key == "shoutout":
        await query.message.reply_text("SHOUTOUT PURCHASED!\n\nYour personal shoutout from BAZRAGOD has been queued.\nAdmin will broadcast it to all fans shortly. 👑")
        try:
            await context.bot.send_message(OWNER_ID, f"SHOUTOUT PURCHASE\n\nFan: @{name} ({uid})\nCost: {cost} pts\n\nRecord a personal shoutout and broadcast to all fans.")
        except Exception:
            pass

    elif item_key == "exclusive_dm":
        if openai_client:
            try:
                response = openai_client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": f"Write a personal sovereign message to @{name} who just spent PARISH points for an exclusive DM. 3 sentences. Make them feel like a true insider of Parish 14. End with a command."},
                    ],
                    max_tokens=150,
                )
                dm_text = response.choices[0].message.content
                await query.message.reply_text(f"EXCLUSIVE MESSAGE FROM MAXIMUS\n\n{dm_text}\n\nParish 14. 👑")
                await maximus_speak(context, uid, dm_text)
            except Exception as e:
                await query.message.reply_text(f"EXCLUSIVE DM\n\nBAZRAGOD sees you. {name}. You are part of the inner circle. Parish 14 Nation. The movement is yours. 👑")
        else:
            await query.message.reply_text(f"EXCLUSIVE DM\n\nBAZRAGOD sees you. {name}. You are part of the inner circle. Parish 14 Nation. 👑")

async def shop_dedication_handler(uid, text, update, context):
    if not context.user_data.get("shop_dedication"):
        return False
    context.user_data.pop("shop_dedication", None)
    name = get_username(update)
    cost = POINT_SHOP_ITEMS["dedication"][1]

    if not deduct_points(uid, cost, "shop_dedication"):
        conn = get_db()
        cur  = conn.cursor()
        try:
            cur.execute("SELECT points FROM fans WHERE telegram_id = %s", (uid,))
            ...
            release_db(conn)
        await update.message.reply_text(f"Not enough points. Need {cost:,}, you have {pts:,}.")
        return True

    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("INSERT INTO dedications (telegram_id, username, message) VALUES (%s, %s, %s) RETURNING id", (uid, name, text))
        ded_id = cur.fetchone()[0]
        conn.commit()
    finally:
        release_db(conn)

    await update.message.reply_text(
        f"DEDICATION QUEUED!\n\nRef #{ded_id}\n\nMAXIMUS will announce your dedication on BazraGod Radio during the next session.\n\nParish 14 Nation. 👑",
        reply_markup=main_menu,
    )
    return True

def _get_pending_dedication():
    conn = get_db()
    cur  = conn.cursor()
    try:
...
async def economy_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid  = update.effective_user.id
    conn = get_db()
    cur  = conn.cursor()
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM fans WHERE points > %s", (pts,))
        rank = cur.fetchone()[0] + 1
        cur.execute("SELECT SUM(pts) FROM point_log WHERE telegram_id = %s AND category = 'earn' AND logged_at > NOW() - INTERVAL '7 days'", (uid,))
        weekly_earn = cur.fetchone()[0] or 0
        cur.execute("SELECT SUM(pts) FROM point_log WHERE telegram_id = %s AND category = 'spend'", (uid,))
        total_spent = abs(cur.fetchone()[0] or 0)
        cur.execute("SELECT amount, multiplier, ends_at FROM stakes WHERE telegram_id = %s AND status = 'active' ORDER BY ends_at", (uid,))
        stakes = cur.fetchall()
        cur.execute("SELECT COUNT(*) FROM nft_ownership WHERE telegram_id = %s", (uid,))
        nft_count = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM auction_bids WHERE telegram_id = %s", (uid,))
        bids_placed = cur.fetchone()[0]
        multiplier = get_multiplier(uid)
        streak = get_fan_streak(uid)
        streak_badge = ""
    ...
    next_rank = get_next_rank_info(pts)
    station   = get_station_rank(pts)
    stake_text = ""
    for amount, mult, ends_at in stakes:
        days_left = max(0, (ends_at - datetime.now()).days)
        stake_text += f"\n  Staked: {amount:,} pts | {mult}x | {days_left}d left"

    await update.message.reply_text(
        f"PARISH 14 ECONOMY DASHBOARD\n\n"
        f"Balance:       {pts:,} PARISH pts\n"
        f"Global Rank:   #{rank}\n"
        f"Nation Tier:   {tier}\n"
        f"Station Rank:  {station}\n"
        f"Multiplier:    {multiplier}x\n"
        f"Streak:        {streak} days {streak_badge}\n\n"
        f"ACTIVITY\n"
        f"Earned this week: {weekly_earn:,} pts\n"
        f"Total spent:      {total_spent:,} pts\n"
        f"NFTs owned:       {nft_count}\n"
        f"Bids placed:      {bids_placed}\n"
        f"Active stakes:    {len(stakes)}{stake_text}\n\n"
...
async def now_playing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur  = conn.cursor()
    try:
        cur.execute("SELECT title, plays, likes, donations FROM songs ORDER BY plays DESC LIMIT 1")
        ...
        cur.execute("SELECT COUNT(*) FROM dedications WHERE played = FALSE")
        pending_deds = cur.fetchone()[0]
    finally:
        release_db(conn)
    ...
    recently_played = "\n".join([f"  {r[0]}" for r in recent]) if recent else "  No history yet"
    top_flags = " ".join([f"{c[0]}" for c in top_countries]) if top_countries else "Worldwide"

    await update.message.reply_text(
        f"BAZRAGOD RADIO — NOW PLAYING\n\n"
        f"DJ on air:     {dj['emoji']} {dj['name']}\n"
        f"Time:          {now}\n"
        f"Listeners:     {listeners} tuned in\n"
        f"Locations:     {top_flags}\n"
        f"Playlist:      {len(playlist)} items queued\n"
        f"Dedications:   {pending_deds} pending\n\n"
        f"TOP SONG: {top_song[0] if top_song else 'Loading...'}\n"
        f"         {top_song[2]:,} plays {calculate_heat(top_song[2], top_song[3], top_song[1]) if top_song else ''}\n\n"
        f"RECENTLY PLAYED:\n{recently_played}\n\n"
        f"YOUR STREAK: {streak} days {streak_badge}\n\n"
        f"Tap BazraGod Radio to tune in.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Tune In Now", callback_data="radio:next")]]),
    )

# ╔══════════════════════════════════════════════════════════════╗
# ║              📡 DAILY DRIP + TIER DIVIDENDS                  ║
# ╚══════════════════════════════════════════════════════════════╝

async def daily_drip_task():
    while True:
        try:
            now = datetime.now()
            if now.hour == 9 and now.minute < 5:
                ...
                cur  = conn.cursor()
                try:
                    cur.execute("SELECT telegram_id, tier, is_supporter FROM fans WHERE tier = 'Nation Elite' OR tier = 'Parish Legend'")
                    elite_fans = cur.fetchall()
                finally:
                    release_db(conn)
                for uid, tier, is_sup in elite_fans:
                    pts = award_points(uid, "start", category="earn")
                    conn2 = get_db()
                    cur2  = conn2.cursor()
                    try:
                        cur2.execute("UPDATE fans SET points = points + %s WHERE telegram_id = %s", (DAILY_DRIP_AMOUNT, uid))
                        cur2.execute("INSERT INTO point_log (telegram_id, action, pts, category) VALUES (%s, 'daily_drip', %s, 'earn')", (uid, DAILY_DRIP_AMOUNT))
                        conn2.commit()
                    finally:
                        release_db(conn2)
                    try:
                        await telegram_app.bot.send_message(uid, f"DAILY DRIP\n\n+{DAILY_DRIP_AMOUNT} PARISH pts deposited.\n\nNation Elite dividend. Every day. Parish 14 rewards loyalty. 👑")
                    except Exception:
                        pass
        except Exception as e:
            print(f"Daily drip error: {e}")
        await asyncio.sleep(300)

# ╔══════════════════════════════════════════════════════════════╗
# ║              🎙️  RADIO WITH STREAK TRACKING                  ║
# ╚══════════════════════════════════════════════════════════════╝

async def radio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid  = update.effective_user.id
    name = get_username(update)
    streak = update_listener(uid)

    streak_msg = ""
    for milestone, bonus in sorted(STREAK_BONUSES.items()):
        if streak == milestone:
            award_points(uid, "streak_bonus", name)
            conn = get_db()
            cur  = conn.cursor()
            try:
                cur.execute("UPDATE fans SET points = points + %s WHERE telegram_id = %s", (bonus, uid))
                cur.execute("INSERT INTO point_log (telegram_id, action, pts, category) VALUES (%s, 'streak_bonus', %s, 'earn')", (uid, bonus))
                conn.commit()
            finally:
                release_db(conn)
            badge = STREAK_BADGES.get(milestone, "")
            streak_msg = f"\n\n{badge} STREAK BONUS!\n+{bonus} pts for {milestone}-day listening streak!"
            break

    listeners = get_listener_count()
    pts       = award_points(uid, "radio", name)
    await _play_next_for_user(uid, name, pts, listeners, update.message, context, streak_msg)
...
async def _play_next_for_user(uid, name, pts, listeners, msg, context, extra_msg=""):
    ...
    if item["type"] == "ad":
        pending_ded = _get_pending_dedication()
        if pending_ded:
            await dj_speak(context, uid, f"Shoutout to {pending_ded['username']}. {pending_ded['message']}. BazraGod Radio. Parish 14.")
            await msg.reply_text(
                f"RADIO DEDICATION\n\nFor {pending_ded['username']}:\n{pending_ded['message']}\n\nBazraGod Radio {now}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Continue Radio", callback_data="radio:next")]]),
            )
        else:
            await msg.reply_text(
                f"BazraGod Radio {now}\n\n{random.choice(AD_MESSAGES)}\n\n{listeners} listeners  {streak} day streak {streak_badge}{extra_msg}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Continue Radio", callback_data="radio:next")]]),
            )
        return
    ...
    if item["type"] == "beat" and item["file_id"]:
        ...
        await msg.reply_audio(
            item["file_id"],
            caption=f"BAZRAGOD BEAT {item['title']}\nBazraGod Radio {now}\n\n{listeners} listeners  Streak: {streak} days {streak_badge}\n\n+{pts} pts{extra_msg}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Next Track", callback_data="radio:next")]]),
        )
        return
    ...
    if item["type"] == "song" and item["file_id"]:
        ...
        multiplier = get_multiplier(uid)
        mult_badge = f"  {multiplier}x stake" if multiplier > 1.0 else ""
        await msg.reply_audio(
            file_id,
            caption=(
                f"BazraGod Radio {now}\n\n"
                f"{title}  {rot_badge}\nBAZRAGOD\n\n"
                f"{heat}  {plays:,} plays  {likes} likes\n\n"
                f"{listeners} listeners  Streak: {streak} days {streak_badge}\n\n"
                f"+{pts} pts{mult_badge}{extra_msg}"
            ),
        ...
        _log_radio_history(file_id, title)
...
main_menu = ReplyKeyboardMarkup(
    [
        ...
        ["Now Playing",       "Mood Radio"],
        ["Lyric Cipher",      "Top Charts"],
        ["Auction House",     "NFT Drops"],
        ["Point Staking",     "Point Shop"],
        ["Economy Dashboard", "Secret Vault"],
        ["Submit Track",      "Leaderboard"],
        ["My Points",         "My Profile"],
        ["Daily Mission",     "Support Artist"],
        ["Social",            "Music Store"],
        ["Parish 14",         "Wisdom"],
        ["Fitness",           "Share Location"],
        ["Refer a Friend",    "Fan Radar"],
        ["Booking",           "Voice Wall"],
        ["Astro Reading",     "Language"],
        ["MAXIMUS AI",        "Help"],
    ],
...
async def maximus_speak(context, chat_id, text):
    ...
    try:
        ...
        buf = BytesIO(response.content); buf.name = "maximus.ogg"
...
async def dj_speak(context, chat_id, text):
    ...
    try:
        ...
        buf = BytesIO(response.content); buf.name = "dj.ogg"
        await context.bot.send_voice(chat_id=chat_id, voice=buf)
    except Exception as e:
...
async def like_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if song_id == 0:
        await query.answer("Tap a song from the catalog to like it!", show_alert=False); return
    uid  = query.from_user.id
    name = query.from_user.username or query.from_user.first_name
    conn = get_db()
    cur  = conn.cursor()
...
async def drops_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        release_db(conn)
    if not rows:
        await update.message.reply_text("No drops yet.")
        return
...
async def vault_item_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if already_unlocked or fan_points >= req_pts:
        ...
        try:
            ...
            release_db(conn)
        await query.message.reply_audio(file_id, caption=f"VAULT UNLOCKED\n\n{title}\nBAZRAGOD Exclusive\n\nParish 14 Nation.")
        await maybe_prophecy(uid, name, context)
    else:
        needed = req_pts - fan_points
        await query.answer(f"Need {needed:,} more points.", show_alert=True)

async def submit_track(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    submission_sessions[uid] = True
    await update.message.reply_text(
        "SUBMIT YOUR TRACK\n\nSend your audio file now with caption:\nArtistName - SongTitle\n\n"
...
async def mood_radio_handler(uid, text, update, context):
    ...
    if song:
        ...
        await update.message.reply_audio(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Like", callback_data=f"like:{sid}"), InlineKeyboardButton("Donate", callback_data=f"donate:{sid}")]]),
        )
        await maybe_prophecy(uid, name, context)
    return True
...
async def request_track(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"REQUEST A TRACK\n\nType the song name.\n\nAvailable:\n{song_list}\n\nMAXIMUS will spin it on air.\n\nType /cancel to go back.")
...
async def request_handler(uid, text, update, context):
    ...
    await update.message.reply_text(f"REQUEST QUEUED!\n\nSong: {text}\nRequest #{req_id}\n\nMAXIMUS will spin it on air.\n\n+{pts} pts", reply_markup=main_menu)
    return True
...
async def activate_supporter_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args or not args[0].isdigit():
        await update.message.reply_text("Usage: /activate_supporter <telegram_id>"); return
    fan_id  = int(args[0])
    expires = date.today() + timedelta(days=30)
    conn    = get_db()
    cur     = conn.cursor()
    try:
        cur.execute("UPDATE fans SET is_supporter = TRUE, tier = 'Nation Elite', supporter_expires = %s WHERE telegram_id = %s RETURNING username", (expires, fan_id))
    ...
    if row:
        ...
        try:
            await context.bot.send_message(fan_id, f"PARISH 14 SUPPORTER ACTIVATED!\n\nNation Elite unlocked.\nExpires: {expires.strftime('%B %d, %Y')}\n\nBAZRAGOD sees you.")
        except Exception:
            pass
    else:
...
async def approve_voice_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args or not args[0].isdigit():
        await update.message.reply_text("Usage: /approve_voice <id>"); return
    vid  = int(args[0])
    conn = get_db()
    cur  = conn.cursor()
    try:
...
async def reject_voice_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args or not args[0].isdigit():
        await update.message.reply_text("Usage: /reject_voice <id>"); return
    vid  = int(args[0])
    conn = get_db()
    cur  = conn.cursor()
    try:
...
async def list_voices_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        release_db(conn)
    if not rows:
        await update.message.reply_text("No voice submissions yet."); return
    text = "VOICE WALL\n\n"
    for r in rows:
...
async def upload_classify_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not session:
        await query.message.reply_text("Session expired. Send the file again."); return
...
async def merch_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    if item_key not in MERCH_ITEMS: return
...
async def store_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    if item_key not in STORE_ITEMS: return
...
async def booking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"BOOK BAZRAGOD\n\nContact:\n{BOOKING_EMAIL}\n\nInclude:\nEvent type\nDate and location\nBudget\nContact number\n\nBAZRAGOD is global. Parish 14 Nation.")
...
async def fan_radar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        release_db(conn)
    if not rows:
        await update.message.reply_text("No fans mapped yet. Share your location!"); return
...
async def leaderboard_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if lb_type == "today":
            cur.execute("""
                SELECT f.username, COALESCE(SUM(pl.pts), 0) as pts, f.tier
                FROM fans f LEFT JOIN point_log pl ON f.telegram_id = pl.telegram_id
                    AND pl.logged_at > NOW() - INTERVAL '24 hours'
                GROUP BY f.username, f.tier ORDER BY pts DESC LIMIT 10
            """)
            ...
            cur.execute("SELECT username, points, tier FROM fans ORDER BY points DESC LIMIT 10")
...
async def my_points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"YOUR STATS\n\n"
        f"Points:       {pts:,} PARISH pts\n"
        f"Global Rank:  #{rank}\n"
        f"Nation Tier:  {tier}\n"
        f"Station Rank: {station}\n"
        f"Invites:      {invites}\n"
        f"Multiplier:   {multiplier}x{mult_badge}\n"
        f"Streak:       {streak} days {streak_badge}\n\n"
        f"NEXT: {next_rank}\n\nKeep grinding."
    )
...
async def my_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur  = conn.cursor()
    try:
        cur.execute("SELECT username, points, invites, tier, city, country, joined_at, is_supporter, supporter_expires, language FROM fans WHERE telegram_id = %s", (uid,))
        row = cur.fetchone()
        cur.execute("SELECT COUNT(*) FROM fans WHERE points > COALESCE((SELECT points FROM fans WHERE telegram_id = %s), 0)", (uid,))
        ...
        cur.execute("SELECT COUNT(*) FROM nft_ownership WHERE telegram_id = %s", (uid,))
        nft_count = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM stakes WHERE telegram_id = %s AND status = 'active'", (uid,))
        active_stakes = cur.fetchone()[0]
    finally:
        release_db(conn)
    if not row:
    ...
    await update.message.reply_text(
        ...
        f"Invites:      {invites}\nStreak:       {streak} days\n"
        f"Multiplier:   {multiplier}x\nVault Items:  {vault_count}\n"
        f"NFTs Owned:   {nft_count}\nStakes Active:{active_stakes}\n"
...
async def mission_complete_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        row = cur.fetchone()
        if row and row[0]:
            await query.message.reply_text("Already completed today!"); return
...
async def astro_reading(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not openai_client:
        await update.message.reply_text("ASTRO READING\n\nMAXIMUS offline. OPENAI_API_KEY required."); return
    uid = update.effective_user.id
...
async def astro_input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid  = update.effective_user.id
    name = get_username(update)
    if uid not in astro_sessions: return False
...
async def send_weekly_intel():
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM auctions WHERE status = 'active'");                               active_auctions = cur.fetchone()[0]
        cur.execute("SELECT SUM(amount) FROM auction_bids WHERE bid_at > NOW() - INTERVAL '7 days'");      auction_vol     = cur.fetchone()[0] or 0
        cur.execute("SELECT COUNT(*) FROM nft_ownership WHERE acquired_at > NOW() - INTERVAL '7 days'");   nft_sold        = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM stakes WHERE status = 'active'");                                 active_stakes   = cur.fetchone()[0]
        cur.execute("SELECT SUM(amount) FROM stakes WHERE status = 'active'");                              staked_total    = cur.fetchone()[0] or 0
    ...
    report = (
        ...
        f"Auction Volume:      {auction_vol:,} pts\n"
        f"NFTs Sold:           {nft_sold}\n"
        f"Active Stakes:       {active_stakes}\n"
        f"Total Staked:        {staked_total:,} pts\n\n"
        f"TOP COUNTRIES:\n{countries_text}\n\n"
        f"TOP FANS:\n{fans_text}\n\n"
        f"TOP SONGS:\n{songs_text}\n\n"
        f"MAXIMUS INTEL SYSTEM v16.000"
    )
    try:
        await telegram_app.bot.send_message(OWNER_ID, report)
        await maximus_speak_direct(telegram_app.bot, OWNER_ID,
            f"Weekly intel ready. {new_fans} new fans. {total_fans} total soldiers. Economy active. The movement grows.")
    except Exception as e:
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    playlist = get_cached_playlist()
    await update.message.reply_text(
        f"I.A.A.I.M.O ADMIN PANEL v16.000\n\n"
        f"Channel Radio: {'ACTIVE' if radio_loop_running else 'STANDBY'}\n"
        f"Playlist: {len(playlist)} items\n\n"
        f"ANALYTICS\n/stats  /radar  /weekly\n\n"
        f"CONTENT\n/list_songs  /delete_song id\n/list_beats  /delete_beat id\n/list_drops  /delete_drop id\n\n"
        f"ECONOMY\n/create_auction title desc starting_bid hours\n/create_nft title desc editions price_pts\n/list_auctions\n/list_nfts\n/list_stakes\n\n"
        f"VAULT\n/list_vault\n/delete_vault id\n\n"
...
async def start_radio_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    if not RADIO_CHANNEL_ID:
        await update.message.reply_text("RADIO_CHANNEL_ID not set.\n\nAdd in Railway:\nRADIO_CHANNEL_ID = -100XXXXXXXXX"); return
    if radio_loop_running:
        await update.message.reply_text("Channel radio loop is already running."); return
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM nft_drops WHERE status = 'active'");                              nfts        = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM stakes WHERE status = 'active'");                                 stakes      = cur.fetchone()[0]
        cur.execute("SELECT SUM(amount) FROM stakes WHERE status = 'active'");                              staked      = cur.fetchone()[0] or 0
        cur.execute("SELECT COUNT(*) FROM fans WHERE is_supporter = TRUE");                                 supporters  = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM radio_sessions WHERE last_ping > NOW() - INTERVAL '30 minutes'"); live        = cur.fetchone()[0]
    ...
    await update.message.reply_text(
        ...
        f"Songs: {total_songs}\n\n"
        f"ECONOMY:\n"
        f"Active Auctions: {auctions}\n"
        f"Active NFT Drops: {nfts}\n"
        f"Active Stakes: {stakes}\n"
        f"Total Staked: {staked:,} pts\n\n"
        f"Top Song: {top_song[0] if top_song else 'None'}\n"
...
async def radar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        release_db(conn)
    if not rows:
        await update.message.reply_text("No fan locations yet."); return
...
async def premiere_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args or not args[0].isdigit():
        await update.message.reply_text("Usage: /premiere <song_id>"); return
    song_id = int(args[0])
    conn    = get_db()
    cur     = conn.cursor()
    try:
        ...
        release_db(conn)
    if not row:
        await update.message.reply_text("Song not found."); return
    title = row[0]
    msg   = f"WORLD PREMIERE\n\nYou are hearing '{title}' for the first time.\n\nBAZRAGOD drops it here first.\nNo label. No middleman. Parish 14 Nation."
...
async def list_auctions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("SELECT id, title, current_bid, top_username, ends_at, status FROM auctions ORDER BY id DESC LIMIT 20")
        rows = cur.fetchall()
    finally:
        release_db(conn)
    if not rows:
        await update.message.reply_text("No auctions yet."); return
    text = "AUCTIONS\n\n"
    for r in rows:
        text += f"[{r[0]}] {r[1]}\n{r[2]:,} pts  Winner: {r[3] or 'None'}\nEnds: {r[4].strftime('%d/%m %H:%M')}  {r[5]}\n\n"
    await update.message.reply_text(text)

async def list_nfts_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("SELECT id, title, editions_left, price_pts, status FROM nft_drops ORDER BY id DESC LIMIT 20")
        rows = cur.fetchall()
    finally:
        release_db(conn)
    if not rows:
        await update.message.reply_text("No NFT drops yet."); return
    text = "NFT DROPS\n\n"
    for r in rows:
        text += f"[{r[0]}] {r[1]}\n{r[2]} editions left  {r[3]:,} pts  {r[4]}\n\n"
    await update.message.reply_text(text)

async def list_stakes_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    conn = get_db()
    cur  = conn.cursor()
    try:
        cur.execute("SELECT s.telegram_id, f.username, s.amount, s.days, s.multiplier, s.ends_at FROM stakes s JOIN fans f ON s.telegram_id = f.telegram_id WHERE s.status = 'active' ORDER BY s.amount DESC LIMIT 20")
        rows = cur.fetchall()
        cur.execute("SELECT SUM(amount) FROM stakes WHERE status = 'active'")
        total = cur.fetchone()[0] or 0
    finally:
        release_db(conn)
    if not rows:
        await update.message.reply_text("No active stakes."); return
    text = f"ACTIVE STAKES\n\nTotal staked: {total:,} pts\n\n"
    for uid, username, amount, days, mult, ends_at in rows:
        text += f"@{username} {amount:,} pts {days}d {mult}x\nUnlocks: {ends_at.strftime('%d/%m/%Y')}\n\n"
    await update.message.reply_text(text)
...
async def list_vault_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        release_db(conn)
    if not rows:
        await update.message.reply_text("Vault is empty."); return
...
async def delete_vault_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args or not args[0].isdigit():
        await update.message.reply_text("Usage: /delete_vault <id>"); return
    row_id = int(args[0])
    conn   = get_db()
    cur    = conn.cursor()
    try:
...
async def list_submissions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        release_db(conn)
    if not rows:
        await update.message.reply_text("No submissions yet."); return
...
async def approve_submission_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args or not args[0].isdigit():
        await update.message.reply_text("Usage: /approve_submission <id>"); return
    sub_id = int(args[0])
    conn   = get_db()
    cur    = conn.cursor()
    try:
    ...
    if row:
        ...
        try:
            await context.bot.send_message(row[0], f"YOUR TRACK WAS APPROVED!\n\n{row[1]} {row[2]}\n\nNow live on BazraGod Radio.\n\nParish 14 Nation. BAZRAGOD.")
        except Exception:
            pass
    else:
...
async def reject_submission_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args or not args[0].isdigit():
        await update.message.reply_text("Usage: /reject_submission <id>"); return
    sub_id = int(args[0])
    conn   = get_db()
    cur    = conn.cursor()
    try:
...
async def shoutout_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("Usage: /shoutout @username"); return
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    for d in [pending_broadcasts, astro_sessions, mood_sessions, cipher_sessions, upload_sessions, request_sessions, submission_sessions, dedication_sessions, auction_bid_sessions]:
        d.pop(uid, None)
    context.user_data["ai_active"]         = False
    context.user_data["voice_wall_active"] = False
    context.user_data["staking_custom"]    = False
    context.user_data["shop_dedication"]   = False
    await update.message.reply_text("Cancelled.", reply_markup=main_menu)

async def menu_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    for d in [astro_sessions, mood_sessions, cipher_sessions, upload_sessions, request_sessions, submission_sessions, auction_bid_sessions]:
        d.pop(uid, None)
    context.user_data["ai_active"]         = False
...
async def list_songs_cmd(update, context):
    ...
    for r in rows:
        text += f"[{r[0]}] {rb.get(r[5],'')} {r[1]}\n{calculate_heat(r[3],r[4],r[2])} {r[2]:,} plays  {r[3]} likes\n"
...
async def _delete_from(update, context, table):
    args = context.args
    if not args or not args[0].isdigit():
        await update.message.reply_text(f"Usage: /delete_{table[:-1]} <id>"); return
    row_id = int(args[0])
    conn   = get_db()
    cur    = conn.cursor()
    try:
...
async def router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in auction_bid_sessions:
        if await auction_bid_handler(uid, text, update, context): return
    if context.user_data.get("staking_custom"):
        if await stake_custom_handler(uid, text, update, context): return
    if context.user_data.get("shop_dedication"):
        if await shop_dedication_handler(uid, text, update, context): return
    ...
    routes = {
        ...
        "NFT Drops":         nft_drops,
        "Point Staking":     staking,
        "Point Shop":        point_shop,
        "Economy Dashboard": economy_dashboard,
...
telegram_app.add_handler(CommandHandler("create_nft",          create_nft_cmd))
telegram_app.add_handler(CommandHandler("list_auctions",       list_auctions_cmd))
telegram_app.add_handler(CommandHandler("list_nfts",           list_nfts_cmd))
telegram_app.add_handler(CommandHandler("list_stakes",         list_stakes_cmd))
...
telegram_app.add_handler(CallbackQueryHandler(auction_bid_cb,      pattern="^auction_bid:"))
telegram_app.add_handler(CallbackQueryHandler(auction_bid_cb,      pattern="^auction_mybids"))
telegram_app.add_handler(CallbackQueryHandler(nft_buy_cb,          pattern="^nft_buy:"))
telegram_app.add_handler(CallbackQueryHandler(nft_buy_cb,          pattern="^nft_collection"))
...
def start_bot():
    ...
    loop.create_task(process_stake_maturity())
    loop.create_task(daily_drip_task())
    loop.run_forever()
...
def health():
    playlist = get_cached_playlist()
    return f"I.A.A.I.M.O ONLINE v16.000 PARISH 14 NATION RADIO {'BROADCASTING' if radio_loop_running else 'STANDBY'} PLAYLIST {len(playlist)} items", 200
...
if __name__ == "__main__":
    ...
    playlist = get_cached_playlist()
    print("I.A.A.I.M.O MISERBOT v16.000")
    print("SOVEREIGN ECONOMY ENGINE ACTIVE")
    print(f"Songs:    {len(SEED_SONGS)}")
    print(f"Beats:    {len(SEED_BEATS)}")
    print(f"Drops:    {len(SEED_DROPS)}")
    print(f"Playlist: {len(playlist)} items")
    print("Auction House:    ACTIVE")
    print("NFT Drops:        ACTIVE")
    print("Point Staking:    ACTIVE")
    print("Point Shop:       ACTIVE")
    print("Listener Streaks: ACTIVE")
    print("Daily Drip:       ACTIVE")
    print("Radio Dedications:ACTIVE")
    print("Status: ONLINE")

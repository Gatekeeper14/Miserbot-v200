...
async def merch_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: await app.bot.send_message(OWNER_ID, f"MERCH ORDER\nFan: @{name} ({uid})\nItem: {item_name}\nPrice: ${price}")
    except Exception: pass
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in skill_sessions:
        ...
        if step == "skill_name":
            ...
            try: await app.bot.send_message(OWNER_ID, f"NEW SKILL SUBMISSION #{sid}\n\nFan: @{name} ({uid})\nSkill: {skill_name}\nDescription: {text}")
            except Exception: pass
            return
...
def main():
    ...
    app.add_handler(CommandHandler("beats", cmd_beats))
    ...
    app.add_handler(CommandHandler("premiere", cmd_premiere))
    app.add_handler(CommandHandler("list_songs", cmd_list_songs))
    app.add_handler(CommandHandler("delete_song", cmd_delete_song))
    app.add_handler(CommandHandler("list_vault", cmd_list_vault))
    app.add_handler(CommandHandler("delete_vault", cmd_delete_vault))
    app.add_handler(CommandHandler("vault_unlock", cmd_vault_unlock))
    app.add_handler(CommandHandler("unlock_download", cmd_unlock_download))
    app.add_handler(CommandHandler("activate_supporter", cmd_activate_supporter))
    app.add_handler(CommandHandler("add_event", cmd_add_event))

    app.add_handler(CallbackQueryHandler(lang_cb, pattern="^lang:"))
    app.add_handler(CallbackQueryHandler(entry_step2, pattern="^entry:step2"))
    app.add_handler(CallbackQueryHandler(entry_step3, pattern="^entry:step3"))
    app.add_handler(CallbackQueryHandler(entry_agreed, pattern="^entry:agreed"))
    app.add_handler(CallbackQueryHandler(entry_gate_done, pattern="^entry:gate_done"))
    app.add_handler(CallbackQueryHandler(play_song_cb, pattern="^song:"))
    app.add_handler(CallbackQueryHandler(like_cb, pattern="^like:"))
    app.add_handler(CallbackQueryHandler(beat_cb, pattern="^beat:"))
    app.add_handler(CallbackQueryHandler(buy_beat_cb, pattern="^buy_beat:"))
    app.add_handler(CallbackQueryHandler(buy_song_cb, pattern="^buy_song:"))
    app.add_handler(CallbackQueryHandler(cart_add_cb, pattern="^cart_add:"))
    app.add_handler(CallbackQueryHandler(cart_remove_cb, pattern="^cart_remove:"))
    app.add_handler(CallbackQueryHandler(cart_clear_cb, pattern="^cart_clear"))
    app.add_handler(CallbackQueryHandler(cart_checkout_cb, pattern="^cart_checkout"))
    app.add_handler(CallbackQueryHandler(radio_next_cb, pattern="^radio:next"))
    app.add_handler(CallbackQueryHandler(vault_item_cb, pattern="^vault:"))
    app.add_handler(CallbackQueryHandler(vault_pay_cb, pattern="^vault_pay:"))
    app.add_handler(CallbackQueryHandler(store_cb, pattern="^store:"))
    app.add_handler(CallbackQueryHandler(service_cb, pattern="^service:"))
    app.add_handler(CallbackQueryHandler(merch_cb, pattern="^merch:"))
    app.add_handler(CallbackQueryHandler(leaderboard_cb, pattern="^lb:"))
    app.add_handler(CallbackQueryHandler(mission_complete_cb, pattern="^mission:"))
    app.add_handler(CallbackQueryHandler(volunteer_cb, pattern="^volunteer:"))
    app.add_handler(CallbackQueryHandler(upload_cb, pattern="^upload:"))
    app.add_handler(CallbackQueryHandler(action_cb, pattern="^action:"))
